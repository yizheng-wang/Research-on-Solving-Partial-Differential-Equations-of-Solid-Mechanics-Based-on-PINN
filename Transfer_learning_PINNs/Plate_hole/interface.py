...
# 定义文件夹路径和精确解文件路径
folders = [ "circle", "elipse2circle_full_finetuning", "elipse2circle_lightweight", "elipse2circle_lora/r=4"]
# folders_in = [ 'BINN', 'KINN_BINN',  "PINN", "KINN_PINN", "DEM", "KINN_DEM",]
labels = ['y', 'y', 'x']
base_path = "./results/"  # 替换为实际的基础路径
exact_files = {
    "x0disy": os.path.join(base_path, "circle", "exact_x0disy.npy"),
    "x0mise": os.path.join(base_path, "circle", "exact_x0mise.npy"),
    "y0disx": os.path.join(base_path, "circle", "exact_y0disx.npy")
}
...
def plot_comparison(variable, exact_data, method_data, labels, title, xlabel):
    ...
    plt.grid(True)
    plt.savefig('./pic/' + title + '_circle.pdf', dpi=300)
    plt.show()

# 读取各个方法的数据并比较
for variable, xlabel in zip(["x0disy", "x0mise", "y0disx"], labels):
    exact_data = exact_solutions[variable]
    method_data = [load_data(folders[i], 'DEM'+'_'+variable+'.npy') for i in range(len(folders))]
...
# 定义文件夹路径和精确解文件路径
folders = [ "elipse", "circle2elipse_full_finetuning" , "circle2elipse_lightweight", "circle2elipse_lora/r=4"]
# folders_in = [ 'BINN', 'KINN_BINN',  "PINN", "KINN_PINN", "DEM", "KINN_DEM",]
labels = ['y', 'y', 'x']
base_path = "./results/"  # 替换为实际的基础路径
exact_files = {
    "x0disy": os.path.join(base_path, "elipse", "exact_x0disy.npy"),
    "x0mise": os.path.join(base_path, "elipse", "exact_x0mise.npy"),
    "y0disx": os.path.join(base_path, "elipse", "exact_y0disx.npy")
}
...
def plot_comparison(variable, exact_data, method_data, labels, title, xlabel):
    ...
    plt.figure(figsize=(10, 8))
    plt.scatter(exact_data[0,:], exact_data[1,:], color = 'r', label="Reference")
    colors = [ 'green' , 'blue', 'orange', 'purple']
    linestyles = ['--', '-', '--', '-','--', '-']
    # markers = ['o', 's', 'o', 's', 'o', 's']
    ...
    for data, label, color, linestyle in zip(method_data, labels, colors, linestyles):
        data = sparse_data(data)
        plt.scatter(data[0, :], data[1, :], color=color, label=label, linestyle=linestyle)
    ...
    plt.legend(fontsize=19)
    plt.grid(True)
    plt.savefig('./pic/' + title + '_elipse.pdf', dpi=300)
    plt.show()
