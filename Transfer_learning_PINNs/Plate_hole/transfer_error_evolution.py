...
# 定义文件夹路径和文件名
folders = ["circle", "elipse2circle_full_finetuning", "elipse2circle_lightweight", "elipse2circle_lora/r=100"]
...
def plot_loss_comparison_smoothed(filenames, title, y_label, smooth_window=500):
    ...
    plt.title('Circle: ' + title, fontsize=25)
    plt.legend(fontsize=18)
    plt.grid(True, which="both", ls="--")
    plt.savefig('./pic/' + title + '_circle_transfer.png', dpi=100)
    plt.show()
...
folders = [ "elipse", "circle2elipse_full_finetuning" , "circle2elipse_lightweight", "circle2elipse_lora/r=100"]
...
def plot_loss_comparison_smoothed(filenames, title, y_label, smooth_window=500):
    ...
    plt.ylabel(y_label, fontsize=14)
    plt.yscale('log')  # y轴使用对数刻度
    plt.title('Elipse: ' + title, fontsize=25)
    plt.legend(fontsize=18)
    plt.grid(True, which="both", ls="--")
    plt.savefig('./pic/' + title + '_elipse_transfer.png', dpi=100)
    plt.show()
