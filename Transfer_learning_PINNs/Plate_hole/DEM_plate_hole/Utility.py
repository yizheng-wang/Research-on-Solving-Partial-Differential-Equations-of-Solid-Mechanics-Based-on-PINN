from DEM_plate_hole.importlib import *
...
def write_vtk_v2p(filename, dom, U, U_mag, S11, S12, S13, S22, S23, S33, E11, E12, E13, E22, E23, E33, SVonMises, U_mag_exact, MISES_exact, U_mag_error, MISES_error): #已经将输出的感兴趣场进行了分类VTK导出
    xx = np.ascontiguousarray(dom[:, 0])
    yy = np.ascontiguousarray(dom[:, 1])
    zz = np.ascontiguousarray(dom[:, 2]) # 点的VTK
    U_mag_exact = np.ascontiguousarray(U_mag_exact)
    MISES_exact = np.ascontiguousarray(MISES_exact)
    pointsToVTK(filename, xx, yy, zz, data={"displacementX": U[0], "displacementY": U[1], "displacementZ": U[2],\
                                            "S-VonMises": SVonMises, "U-mag": U_mag, \
                                                "U_mag_exact": U_mag_exact, "MISES_exact": MISES_exact,\
                                                 "U_mag_error": U_mag_error, "MISES_error": MISES_error,\
                                               "S11": S11, "S12": S12, "S13": S13, \
                                               "S22": S22, "S23": S23, "S33": S33, \
                                               "E11": E11, "E12": E12, "E13": E13, \
                                               "E22": E22, "E23": E23, "E33": E33\
