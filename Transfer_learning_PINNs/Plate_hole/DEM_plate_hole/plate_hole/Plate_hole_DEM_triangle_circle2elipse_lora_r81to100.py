# -*- coding: utf-8 -*-
"""
Created on Wed Jan 29 16:09:17 2025
...
class DeepEnergyMethod:
    # Instance attributes
    def __init__(self, model, numIntType, energy, dim, defect=False, r=0, rank=4):
        # self.data = data
        self.model = MultiLayerNetLoRA(model[0], model[1], model[2], r=rank)
        self.model = self.model.to(dev)
        self.model.load_state_dict(torch.load('./model/model_circle.pth'), strict = False)
    ...
    def train_model(self, data, neumannBC, dirichletBC, boundary_id, LHD, iteration, learning_rate):
        ...
        U_mag_exact = np.load('../../abaqus_reference/elipse/U_mag.npy')[:,1]
        MISES_exact = np.load('../../abaqus_reference/elipse/MISES.npy')[:,1]
        ...
        torch.save(self.model.state_dict(), f'model/model_circle2elipse_lora_r={self.rank}.pth')
    def getUST(self, x):
        '''
...
if __name__ == '__main__':
    r_list = list(range(81, 100))
    r_list = [x for x in r_list if x!=4 and x%4==0]
    for r in r_list:
        path = f'../../results/circle2elipse_lora/r={r}'
        # 检查路径是否存在，如果不存在则创建
        ...
        # ----------------------------------------------------------------------
        dom, boundary_neumann, boundary_dirichlet, bound_id = des.setup_domain_elipse_tri() # 返回的boundary value是一个字典，里面不仅仅有坐标还有力大小，力大小没有标准化
        ...
        dem = DeepEnergyMethod([cf.D_in, cf.H, cf.D_out], 'montecarlo', mat, 2, defect=True, r = cf.r, rank = r)
        # ----------------------------------------------------------------------
        ...
        cf.filename_out = f'../../results/circle2elipse_lora/r={r}'
        # 将坐标点先存起来
        N = dom.shape[0] # abaqus最后一个维度必须是0，就是必须要输入三位坐标才行
        dom_aba = np.hstack((dom[:,:-1], np.zeros((N, 1))))
        np.save('../../abaqus_reference/elipse/coordinate.npy', dom_aba)
        ...
        #*****************************************************************************************************************************************
        util.write_vtk_v2p(cf.filename_out+'/Plate_hole_DEM', dom_aba, U, U_mag, S11, S12, S13, S22, S23, S33, E11, E12, E13, E22, E23, E33,  SVonMises, U_mag_exact, MISES_exact, U_mag_error, MISES_error)
        surUx, surUy, surUz = U
