...
class DeepEnergyMethod:
    ...
    def __init__(self, model, numIntType, energy, dim, defect=False, r=0):
        ...
        self.model.load_state_dict(torch.load('./model/model_circle.pth'))
        ...
        self.lossArray = []
        self.r = r
    def freeze_layers(self, model: nn.Module, train_layer_keywords: list, lr=1e-3):
        """
    ...
    def train_model(self, data, neumannBC, dirichletBC, boundary_id, LHD, iteration, learning_rate):
        ...
        optimizer = self.freeze_layers(self.model, train_layer_keywords=["linear5"], lr=0.001)
        ...
        torch.save(self.model.state_dict(), f'model/model_circle2elipse_lightweight.pth')
    def getUST(self, x):
        '''
...
if __name__ == '__main__':
    ...
    cf.filename_out = '../../results/circle2elipse_lightweight'
    # 将坐标点先存起来
    N = dom.shape[0] # abaqus最后一个维度必须是0，就是必须要输入三位坐标才行
    dom_aba = np.hstack((dom[:,:-1], np.zeros((N, 1))))
    np.save('../../abaqus_reference/circle/coordinate.npy', dom_aba)
...
#  compare the disy and mises(x=0), and disx (y=0)
    datax0 = xlrd.open_workbook("hole_abaqus_x=0_elipse.xlsx") # 获得x=0的数据，需要提取y方向位移以及mise应力
    ...
    datay0 = xlrd.open_workbook("hole_abaqus_y=0_elipse.xlsx") # 获得x=0的数据，需要提取y方向位移以及mise应力
