...
class DeepEnergyMethod:
    ...
    def train_model(self, data, neumannBC, dirichletBC, boundary_id, LHD, iteration, learning_rate):
        ...
        torch.save(self.model.state_dict(), f'model/model_circle.pth')
    def getUST(self, x):
        '''
...
if __name__ == '__main__':
    ...
    shape = [cf.Nx, cf.Ny]
    dxdy = [cf.hx, cf.hy]
    cf.filename_out = '../../results/circle'
    # 将坐标点先存起来
    N = dom.shape[0] # abaqus最后一个维度必须是0，就是必须要输入三位坐标才行
    dom_aba = np.hstack((dom[:,:-1], np.zeros((N, 1))))
    dem.model.load_state_dict(torch.load('./model/model_circle.pth'), strict = False)
