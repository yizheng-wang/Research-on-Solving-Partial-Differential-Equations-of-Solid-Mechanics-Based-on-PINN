...
class DeepEnergyMethod:
    ...
    def __init__(self, model, numIntType, energy, dim, defect=False, r=0):
        ...
        self.model.load_state_dict(torch.load('./model/model_elipse.pth'))
    ...
    def train_model(self, data, neumannBC, dirichletBC, boundary_id, LHD, iteration, learning_rate):
        ...
        U_mag_exact = np.load('../../abaqus_reference/circle/U_mag.npy')[:,1]
        MISES_exact = np.load('../../abaqus_reference/circle/MISES.npy')[:,1]
        ...
        torch.save(self.model.state_dict(), f'model/model_elipse2circle_lightweight.pth')
    def getUST(self, x):
        '''
...
if __name__ == '__main__':
    ...
    # ----------------------------------------------------------------------
    dom, boundary_neumann, boundary_dirichlet, bound_id = des.setup_domain_circle_tri() # 返回的boundary value是一个字典，里面不仅仅有坐标还有力大小，力大小没有标准化
    ...
    cf.filename_out = '../../results/elipse2circle_lightweight'
    # 将坐标点先存起来
...
#  compare the disy and mises(x=0), and disx (y=0)
    datax0 = xlrd.open_workbook("hole_abaqus_x=0_circle.xlsx") # 获得x=0的数据，需要提取y方向位移以及mise应力
    ...
    datay0 = xlrd.open_workbook("hole_abaqus_y=0_circle.xlsx") # 获得x=0的数据，需要提取y方向位移以及mise应力
