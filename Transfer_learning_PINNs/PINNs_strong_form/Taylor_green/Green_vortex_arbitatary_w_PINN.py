...
for w_i in w_list:
    ...
    for epoch in range(epoch_num):
        ...
        if epoch % print_e == 0:
            ...
            start_time = time.time()
    np.save(f'results/PINN/loss_error_results_w_{w:.2f}.npy', results)
    torch.save(model.state_dict(), f'results/PINN/model_taylor_green_w_{w:.2f}.pth')
    ...
    def plot_contour(x, y, data, title, label, filename):
        plt.figure(figsize=(7, 6))  # Ensure square plots
        ...
        plt.savefig(f'./pic/PINN/{filename}_taylor.png')
        plt.show()
        plt.close()
