...
for w_i in w_list:
    ...
    for w_o in w_list:
        ...
        # Model and optimizer
        model = KAN([3, 8,8,8, 2], base_activation=torch.nn.SiLU, grid_size=15, grid_range=[0,  1.0], spline_order=3).cuda()
        model.load_state_dict(torch.load(f'PINNs_strong_form/Taylor_green/results/KINN/model_taylor_green_w_{w_i:.2f}.pth'), strict=False)
        ...
        for epoch in range(epoch_num):
            ...
            if epoch % 1 == 0:
                ...
                start_time = time.time()
        np.save(f'results/KINN_full_finetuning/loss_error_results_wi_{w_i:.2f}_wo_{w_o:.2f}.npy', results)
        torch.save(model.state_dict(), f'results/KINN_full_finetuning/model_taylor_green_wi_{w_i:.2f}_wo_{w_o:.2f}.pth')
        ...
        def plot_contour(x, y, data, title, label, filename):
            ...
            plt.xlabel("x")
            plt.ylabel("y")
            plt.savefig(f'./pic/KINN_full_finetuning/{filename}_taylor.png')
            plt.show()
            plt.close()
