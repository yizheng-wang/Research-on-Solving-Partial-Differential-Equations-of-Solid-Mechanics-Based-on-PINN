...
for w_i in w_list:
    ...
    for w_o in w_list:
        ...
        class MultiLayerNetLoRA(nn.Module):
            def __init__(self, D_in, H, D_out, r=4, alpha=1.0):
                """
                LoRA 版本的多层全连接网络:
        ...
        model = MultiLayerNetLoRA(D_in=3, H=100, D_out=2, r=100, alpha=1.0).to(device)
        ...
        for epoch in range(epoch_num):
            ...
            if epoch % print_e == 0:
                ...
                start_time = time.time()
        np.save(f'results/PINN_lora/r=100/loss_error_results_wi_{w_i:.2f}_wo_{w_o:.2f}.npy', results)
        torch.save(model.state_dict(), f'results/PINN_lora/r=100/model_taylor_green_wi_{w_i:.2f}_wo_{w_o:.2f}.pth')
        ...
        def plot_contour(x, y, data, title, label, filename):
            ...
            plt.xlabel("x")
            plt.ylabel("y")
            plt.savefig(f'./pic/PINN_lora/r=100/{filename}_taylor.png')
            plt.show()
            plt.close()
        ...
        plot_contour(x_plot, y_plot, omega_pred, "Vorticity Contour", "Vorticity", f"vorticity_wi_{w_i:.2f}_wo_{w_o:.2f}_t_{t_plot:.2f}")
        plot_contour(x_plot, y_plot, omega_exact, "Vorticity Contour_exact", "Vorticity_exact", f"vorticity_exac_wi_{w_i:.2f}_wo_{w_o:.2f}_t_{t_plot:.2f}")
