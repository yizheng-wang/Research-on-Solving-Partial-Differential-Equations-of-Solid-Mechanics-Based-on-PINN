...
for w_i in w_list:
    ...
    for w_o in w_list:
        ...
        model.load_state_dict(torch.load(f'./results/PINN/model_taylor_green_w_{w_i:.2f}.pth'))
        
        # 只想训练 model.linear5 和 model.linear4
        optimizer = freeze_layers(model, train_layer_keywords=["linear5"], lr=0.001)
        # optimizer = freeze_layers(model, train_layer_keywords=["linear4", "linear5"], lr=0.001)
        # optimizer = freeze_layers(model, train_layer_keywords=["linear3","linear4", "linear5"], lr=0.001)
        # optimizer = freeze_layers(model, train_layer_keywords=["linear2", "linear3","linear4", "linear5"], lr=0.001)
        # optimizer = freeze_layers(model, train_layer_keywords=["linear1","linear2", "linear3","linear4", "linear5"], lr=0.001)
        ...
        for epoch in range(epoch_num):
            ...
            if epoch % print_e == 0:
                ...
                start_time = time.time()
        np.save(f'results/PINN_light_weight/loss_error_results_wi_{w_i:.2f}_wo_{w_o:.2f}.npy', results)
        torch.save(model.state_dict(), f'results/PINN_light_weight/model_taylor_green_wi_{w_i:.2f}_wo_{w_o:.2f}.pth')
        ...
        def plot_contour(x, y, data, title, label, filename):
            ...
            plt.xlabel("x")
            plt.ylabel("y")
            plt.savefig(f'./pic/PINN_light_weight/{filename}_taylor.png')
            plt.show()
            plt.close()
        
        # Generate plots and save them
        plot_contour(x_plot, y_plot, psi_pred, "Stream Function Contour", "Stream Function", f"stream_function_wi_{w_i:.2f}_wo_{w_o:.2f}_t_{t_plot:.2f}")
        plot_contour(x_plot, y_plot, psi_exact, "Stream Function Contour_exact", "Stream Function_exact", f"stream_function_exact_wi_{w_i:.2f}_wo_{w_o:.2f}_t_{t_plot:.2f}")
        
        plot_contour(x_plot, y_plot, omega_pred, "Vorticity Contour", "Vorticity", f"vorticity_wi_{w_i:.2f}_wo_{w_o:.2f}_t_{t_plot:.2f}")
        plot_contour(x_plot, y_plot, omega_exact, "Vorticity Contour_exact", "Vorticity_exact", f"vorticity_exact_wi_{w_i:.2f}_wo_{w_o:.2f}_t_{t_plot:.2f}")
