...
for w_i in w_list:
    ...
    for w_o in w_list:
        ...
        model = MultiLayerNetLoRA(D_in=3, H=100, D_out=2, r=1, alpha=1.0).to(device)
        ...
        for epoch in range(epoch_num):
            ...
            if epoch % print_e == 0:
                ...
                start_time = time.time()
        np.save(f'results/PINN_lora/r=1/loss_error_results_wi_{w_i:.2f}_wo_{w_o:.2f}.npy', results)
        torch.save(model.state_dict(), f'results/PINN_lora/r=1/model_taylor_green_wi_{w_i:.2f}_wo_{w_o:.2f}.pth')
        ...
        def plot_contour(x, y, data, title, label, filename):
            ...
            plt.xlabel("x")
            plt.ylabel("y")
            plt.savefig(f'./pic/PINN_lora/r=1/{filename}_taylor.png')
            plt.show()
            plt.close()
