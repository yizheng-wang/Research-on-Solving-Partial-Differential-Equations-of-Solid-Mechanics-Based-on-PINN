...
class EnergyModel:
    ...
    def getStoredEnergy(self, u, x):
        ...
        if self.type == 'FGM1': # 最小势能原理，线弹性
            if self.dim == 2:
                return self.ElasticityFGM1(u, x)
        if self.type == 'FGM2': # 最小势能原理，线弹性
            if self.dim == 2:
                return self.ElasticityFGM2(u, x)
    ...
    def ElasticityFGM1(self, ust, x):
        ...
        #得到y，因为y会改变弹性模量的值
        y = x[:,1]
        ratio = 1 - 0.5*torch.cos(torch.pi*(y/1-0.5)).reshape(-1,1)
        # 经过张量分析理论分析，这是对的
        strainEnergy = 0.5 * (self.D11_mat * sxx ** 2  + 2*self.D12_mat * sxx * syy + self.D22_mat * syy ** 2 + self.D33_mat * s2xy ** 2)
        strainEnergy = strainEnergy * ratio
        # 获得应变能密度的泛函
        return strainEnergy
    
    def ElasticityFGM2(self, ust, x):
        duxdxy = grad(ust[:, 0].unsqueeze(1), x, torch.ones(x.size()[0], 1, device=dev), create_graph=True, retain_graph=True)[0]
        duydxy = grad(ust[:, 1].unsqueeze(1), x, torch.ones(x.size()[0], 1, device=dev), create_graph=True, retain_graph=True)[0]        
        dudx = duxdxy[:, 0].unsqueeze(1)
        dudy = duxdxy[:, 1].unsqueeze(1)
        ...
        #得到y，因为y会改变弹性模量的值
        y = x[:,1]
        ratio = 1 - 0.5*torch.cos(torch.pi*y/2).reshape(-1,1)
        # 经过张量分析理论分析，这是对的
        strainEnergy = 0.5 * (self.D11_mat * sxx ** 2  + 2*self.D12_mat * sxx * syy + self.D22_mat * syy ** 2 + self.D33_mat * s2xy ** 2)
        strainEnergy = strainEnergy * ratio
        # 获得应变能密度的泛函
