...
class DeepEnergyMethod1to2:
    ...
    def __init__(self, model, numIntType, energy, dim):
        # self.data = data
        self.model = MultiLayerNetLoRA(model[0], model[1], model[2])
    ...
    def train_model(self, shape, dxdydz, data, neumannBC, dirichletBC, iteration, learning_rate):
        ...
        for t in range(iteration):
            ...
            def closure():
                ...
                L2norm = np.linalg.norm(dis_y_pred - dis_y_iga2)/np.linalg.norm( dis_y_iga2)
                H1norm = np.linalg.norm(Von_pred - von_iga2)/np.linalg.norm(von_iga2)
        ...
        torch.save(self.model.state_dict(), f'model/PINN/model_FGM1to2_lora_r4.pth')
    def getU(self, x):
        x_scale = x/cf.Length
    ...
    def train_model(self, shape, dxdydz, data, neumannBC, dirichletBC, iteration, learning_rate):
        ...
        for t in range(iteration):
            ...
            def closure():
                ...
                L2norm = np.linalg.norm(dis_y_pred - dis_y_iga1)/np.linalg.norm( dis_y_iga1)
                H1norm = np.linalg.norm(Von_pred - von_iga1)/np.linalg.norm(von_iga1)
        ...
        elapsed = time.time() - start_time
        print('Training time: %.4f' % elapsed)
        torch.save(self.model.state_dict(), f'model/PINN/model_FGM2to1_lora_r4.pth')
    def getU(self, x):
        x_scale = x/cf.Length
        u = self.model(x_scale)
        Ux = x[:, 0] * u[:, 0] * (cf.Length-x[:, 0])
...
if __name__ == '__main__':
    ...
    # 获取位移场y和vonmise应力
    dis_y_iga2 = np.load('./output/IGA/const-dist2/v_exact.npy').flatten()
    von_iga2 = np.load('./output/IGA/const-dist2/von_mise.npy').flatten()
    ...
    #                   STEP 2: SETUP MODEL
    # ----------------------------------------------------------------------
    mat2 = md.EnergyModel('FGM2', 2, cf.E, cf.nu)
    #dem = DeepEnergyMethod([cf.D_in, cf.H, cf.D_out], 'simpson', mat, 2)
    dem = DeepEnergyMethod1to2([cf.D_in, cf.H, cf.D_out], 'trapezoidal', mat2, 2)
    # ----------------------------------------------------------------------
    ...
    # error_storage
    np.save("./output/dem_lora/r=4/FGM1to2_MLP_trap_L2_norm.npy", dem.L2_array)
    np.save("./output/dem_lora/r=4/FGM1to2_MLP_trap_H1_norm.npy", dem.H1_array)
    ...
    # 获取位移场y和vonmise应力
    dis_y_iga1 = np.load('./output/IGA/const-dist1/v_exact.npy').flatten()
    von_iga1 = np.load('./output/IGA/const-dist1/von_mise.npy').flatten()
    ...
    #                   STEP 2: SETUP MODEL
    # ----------------------------------------------------------------------
    mat1 = md.EnergyModel('FGM1', 2, cf.E, cf.nu)
    #dem = DeepEnergyMethod([cf.D_in, cf.H, cf.D_out], 'simpson', mat, 2)
    dem = DeepEnergyMethod2to1([cf.D_in, cf.H, cf.D_out], 'trapezoidal', mat1, 2)
    # ----------------------------------------------------------------------
    #                   STEP 3: TRAINING MODEL
    # ----------------------------------------------------------------------
    start_time = time.time()
    ...
    # error_storage
    np.save("./output/dem_lora/r=4/FGM2to1_MLP_trap_L2_norm.npy", dem.L2_array)
    np.save("./output/dem_lora/r=4/FGM2to1_MLP_trap_H1_norm.npy", dem.H1_array)
