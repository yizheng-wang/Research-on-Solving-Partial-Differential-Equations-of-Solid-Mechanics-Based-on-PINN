...
def setup_domain():
    ...
    bcr_u_pts_idx = np.where(dom[:, 0] == x_min + Length)
    bcr_u_pts = dom[bcr_u_pts_idx, :][0]
    bcr_u = np.ones(np.shape(bcl_u_pts)) * [known_right_ux, known_right_uy]

    # Up boundary condition (Neumann BC)
    bcu_t_pts_idx = np.where(dom[:, 1] == y_min + Height)
    bcu_t_pts = dom[bcu_t_pts_idx, :][0]
    bcu_t = np.ones(np.shape(bcu_t_pts)) * [known_up_tx, known_up_ty]

    ax.scatter(dom[:, 0], dom[:, 1], s=0.005, facecolor='blue')
    ax.scatter(bcl_u_pts[:, 0], bcl_u_pts[:, 1], s=0.5, facecolor='red')
    ax.scatter(bcr_u_pts[:, 0], bcr_u_pts[:, 1], s=0.5, facecolor='red')
    ax.scatter(bcu_t_pts[:, 0], bcu_t_pts[:, 1], s=0.5, facecolor='green')
    plt.show()
    # exit()
    boundary_neumann = {
