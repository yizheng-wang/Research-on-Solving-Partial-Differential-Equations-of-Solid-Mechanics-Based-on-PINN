...
class DeepEnergyMethod1to2:
    ...
    def __init__(self, model, numIntType, energy, dim):
        ...
        self.model.load_state_dict(torch.load('./model/PINN/model_FGM1.pth'))
    ...
    def train_model(self, shape, dxdydz, data, neumannBC, dirichletBC, iteration, learning_rate):
        ...
        torch.save(self.model.state_dict(), f'model/PINN/model_FGM1to2_lightweight.pth')
    def getU(self, x):
        x_scale = x/cf.Length
...
class DeepEnergyMethod2to1:
    ...
    def __init__(self, model, numIntType, energy, dim):
        ...
        self.model = MultiLayerNet(model[0], model[1], model[2])
        self.model = self.model.to(dev)
        self.model.load_state_dict(torch.load('./model/PINN/model_FGM2.pth'))
        self.intLoss = IntegrationLoss(numIntType, dim)
        self.energy = energy
        # self.post = PostProcessing(energy, dim)
        self.dim = dim
    ...
    def train_model(self, shape, dxdydz, data, neumannBC, dirichletBC, iteration, learning_rate):
        ...
        elapsed = time.time() - start_time
        print('Training time: %.4f' % elapsed)
        torch.save(self.model.state_dict(), f'model/PINN/model_FGM2to1_lightweight.pth')
    def getU(self, x):
        x_scale = x/cf.Length
        u = self.model(x_scale)
        Ux = x[:, 0] * u[:, 0] * (cf.Length-x[:, 0])
...
if __name__ == '__main__':
    ...
    dem = DeepEnergyMethod1to2([cf.D_in, cf.H, cf.D_out], 'trapezoidal', mat, 2)
    # ----------------------------------------------------------------------
    ...
    # error_storage
    np.save("./output/dem_lightweight/FGM1to2_MLP_trap_L2_norm.npy", dem.L2_array)
    np.save("./output/dem_lightweight/FGM1to2_MLP_trap_H1_norm.npy", dem.H1_array)
    ...
    #dem = DeepEnergyMethod([cf.D_in, cf.H, cf.D_out], 'simpson', mat, 2)
    dem = DeepEnergyMethod2to1([cf.D_in, cf.H, cf.D_out], 'trapezoidal', mat, 2)
    # ----------------------------------------------------------------------
    #                   STEP 3: TRAINING MODEL
    # ----------------------------------------------------------------------
    start_time = time.time()
    ...
    # error_storage
    np.save("./output/dem_lightweight/FGM2to1_MLP_trap_L2_norm.npy", dem.L2_array)
    np.save("./output/dem_lightweight/FGM2to1_MLP_trap_H1_norm.npy", dem.H1_array)
