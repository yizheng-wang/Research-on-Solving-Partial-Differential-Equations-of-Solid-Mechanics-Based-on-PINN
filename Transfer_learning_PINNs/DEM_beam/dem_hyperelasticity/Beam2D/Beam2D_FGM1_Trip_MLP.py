...
class DeepEnergyMethod:
    ...
    def train_model(self, shape, dxdydz, data, neumannBC, dirichletBC, iteration, learning_rate):
        ...
        torch.save(self.model.state_dict(), f'model/PINN/model_FGM1.pth')
    def getU(self, x):
        x_scale = x/cf.Length
...
if __name__ == '__main__':
    ...
    util.write_vtk_v2p("./output/dem/FGM1_IGA_disy", points, dis_y_iga)
    util.write_vtk_v2p("./output/dem/FGM1_MLP_trap_disy", points, disy_pred)
    util.write_vtk_v2p("./output/dem/FGM1_MLP_trap_disy_error", points, dis_y_abs_error)
    
    von_pred = dem.evaluate_von(points).flatten()
    
    von_abs_error = np.abs(von_pred.flatten() - von_iga.flatten())
    util.write_vtk_v2p("./output/dem/FGM1_IGA_von", points, von_iga)
    util.write_vtk_v2p("./output/dem/FGM1_MLP_trap_von", points, von_pred)
    util.write_vtk_v2p("./output/dem/FGM1_MLP_trap_von_error", points, von_abs_error)
    # error_storage
    np.save("./output/dem/FGM1_MLP_trap_L2_norm.npy", dem.L2_array)
    np.save("./output/dem/FGM1_MLP_trap_H1_norm.npy", dem.H1_array)
