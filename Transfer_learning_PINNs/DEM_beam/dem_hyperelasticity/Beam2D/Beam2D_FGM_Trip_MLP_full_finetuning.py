...
class DeepEnergyMethod1to2:
    ...
    def train_model(self, shape, dxdydz, data, neumannBC, dirichletBC, iteration, learning_rate):
        ...
        torch.save(self.model.state_dict(), f'model/PINN/model_FGM1to2_full_finetuning.pth')
    def getU(self, x):
        x_scale = x/cf.Length
...
class DeepEnergyMethod2to1:
    ...
    def train_model(self, shape, dxdydz, data, neumannBC, dirichletBC, iteration, learning_rate):
        ...
        elapsed = time.time() - start_time
        print('Training time: %.4f' % elapsed)
        torch.save(self.model.state_dict(), f'model/PINN/model_FGM2to1_full_finetuning.pth')
    def getU(self, x):
        x_scale = x/cf.Length
        u = self.model(x_scale)
        Ux = x[:, 0] * u[:, 0] * (cf.Length-x[:, 0])
...
if __name__ == '__main__':
    ...
    # error_storage
    np.save("./output/dem_full_finetuning/FGM1to2_MLP_trap_L2_norm.npy", dem.L2_array)
    np.save("./output/dem_full_finetuning/FGM1to2_MLP_trap_H1_norm.npy", dem.H1_array)
...
# 2 to 1
    ...
    # error_storage
    np.save("./output/dem_full_finetuning/FGM2to1_MLP_trap_L2_norm.npy", dem.L2_array)
    np.save("./output/dem_full_finetuning/FGM2to1_MLP_trap_H1_norm.npy", dem.H1_array)
