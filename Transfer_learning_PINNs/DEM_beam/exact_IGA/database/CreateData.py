...
def GRF(N, h, mean=0, variance=1, length_scale = 0.1):
    jitter = 1e-10
    X = np.linspace(0, h, N)[:,None] # 0到2，均匀布点
    K = RBF(X, X, length_scale) # 相当于是高斯随机过程的方差
    L = np.linalg.cholesky(K + jitter*np.eye(N)) # K=L*L.T
    gp_sample = variance*np.dot(L, np.random.normal(size=N))+mean  # 相当于是GP（0,k）的一次随机采样，np.dot(L, np.random.normal(size=N))
...
def createDatabase(num_refinements, model_data):
    ...
    X, gp = GRF(N_traction, beam_length, mean=bound_trac, variance=trac_var*bound_trac, length_scale = trac_scale*beam_length) # 相当于traction进行了高斯随机场的一次采样
    ...
    bound_up = boundary2D("Neumann", 0, "up", u_bound_neu) # up boundary是高斯随机场
    bound_left = boundary2D("Dirichlet", 0, "left", u_bound_dir_fixed) # 左右固定
    ...
    gauss_quad_u = gen_gauss_pts(p+1) # 高斯积分点和权重
...
numPtsU = 2**(num_refinements+4) # x方向撒点数目
numPtsV = 2**(num_refinements+4)//len_ratio # y方向撒点数

N_traction = numPtsU #  上部分力按照撒点的数目给定
N_material = numPtsV
