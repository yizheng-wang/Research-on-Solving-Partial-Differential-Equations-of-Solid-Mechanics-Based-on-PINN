...
for epoch in range(nepoch):
    ...
    def closure():  
        ...
        pred_ra = pred(ra)
        pred_rb = pred(rb)
        loss_dom = torch.norm(dUrdrr+1/r*dUrdr-Ur/r**2)/len(r)
        #loss_b = torch.norm(pred_ra - Ura, 1) + torch.norm(pred_rb - Urb, 1)

        loss = loss_dom# + beta * loss_b
        ...
        if epoch%10==0:
            print(' epoch : %i, the loss : %f, the loss_dom : %f' % \
                  (epoch, loss.data, loss_dom.data))
        return loss
    optim.step(closure)
