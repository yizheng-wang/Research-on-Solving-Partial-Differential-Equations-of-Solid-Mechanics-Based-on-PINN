...
if training_part == 1:    
    ...
    optimp = torch.optim.Adam(params=model_p.parameters(), lr= 0.01)
    scheduler = MultiStepLR(optimp, milestones=[3000, 6000, 9000, 10000, 30000], gamma = 0.5)
...
###    The mix boundary are satisfied by the penalty method
            ...
            fai_y_left = model_p(Xb_y_left) # predict the boundary condition
            ...
            dfaidxdy_y_left = dfaidxdxy_y_left[:, 1].unsqueeze(1)
            loss_y_left = criterion(dfaidxdy_y_left, target_y_left)

            fai_x_left = model_p(Xb_x_left) # predict the boundary condition
            ...
            dfaidydy_x_left = dfaidydxy_x_left[:, 1].unsqueeze(1)
            loss_x_left = criterion(-dfaidydy_x_left, target_x_left)
            
            loss_p =  loss_y_right + loss_x_up + loss_y_up + loss_x_right + loss_y_down + loss_x_down + loss_y_left + loss_x_left
            
            optimp.zero_grad()
            loss_p.backward()
            loss_p_array.append(loss_p.data.cpu())
            if epoch_p%10==0:
                print('epoch  %i, loss: %f, x_right: %f, y_right: %f, x_up: %f, y_up: %f, x_down: %f, y_down: %f, x_left: %f, y_left: %f' %\
                      (epoch_p, loss_p.data, loss_x_right.data, loss_y_right.data, loss_x_up.data, loss_y_up.data\
                       , loss_x_down.data, loss_y_down.data, loss_x_left.data, loss_y_left.data))
            return loss_p
        optimp.step(closure)
        scheduler.step()
        loss_p = loss_p_array[-1]
    torch.save(model_p, './particular_DCEM_nn_%ihole_cir_fix_all_pressure' % len(cir))
