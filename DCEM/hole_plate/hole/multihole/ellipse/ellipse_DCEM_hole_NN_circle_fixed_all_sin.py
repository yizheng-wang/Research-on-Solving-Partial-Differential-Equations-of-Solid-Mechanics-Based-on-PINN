...
import cmath
FEM_node = np.load("ellipse_node_coordinate_abaqus_1hole_all_pressure.npy")
...
# add cir boundary points
    ...
    theta = np.linspace(-np.pi, np.pi, Nb)
    for index, eve_cir in enumerate(cir):
      a = eve_cir[3]
      b = eve_cir[2]
      Eccen = np.sqrt(1-b**2/a**2)
      r = b/np.sqrt(1-(Eccen*np.cos(theta))**2)
      x = r*np.sin(theta) + eve_cir[0]
      y = r*np.cos(theta) + eve_cir[1]
...
write_vtk_v2p("./output/ellipse_DCEM_%ihole_NN_cir_fixed_all_pressure" % len(cir),  dom, pred_sigma_x, pred_sigma_xy, pred_sigma_y, pred_mise)
