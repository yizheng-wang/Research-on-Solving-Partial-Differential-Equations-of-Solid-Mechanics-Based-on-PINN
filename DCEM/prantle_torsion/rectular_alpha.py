...
for i in range(num):
    ...
    def evaluate_Dt(N_test):# 计算Dt
        X_test = dom_data_uniform(N_test_uni)
        fai = pred(X_test)
...
for i in range(num):
    ...
    k = 20
    iteration = np.array(range(0,len(pred_alpha_array_strong_dis[i])))
    plt.plot(iteration[::k], pred_alpha_array_strong_dis[i][::k], linestyle=(0,(3.5, 1.5, 1.5)))
    plt.plot(iteration[::k], pred_alpha_array_minpotential[i][::k], linestyle='--')
    plt.plot(iteration[::k], pred_alpha_array_strong_fai[i][::k], linestyle='-.')
    plt.plot(iteration[::k], pred_alpha_array_mincomplement[i][::k], ls = ':')
    plt.axhline(y=alpha_exact, color='r')
    plt.xlabel('Iteration', fontsize=13)
    plt.ylabel('Alpha', fontsize=13)
    plt.ylim(bottom=0.0, top = alpha_exact*2)
    plt.legend(['Strong: Displacement', 'DEM', 'Strong: Stress function', 'DCEM', 'Exact'])
    plt.show()
    plt.savefig('./PIG/alpha_all_method%f.png' % a)
...
# 相对误差
alpha_relative_error_strong_dis = np.zeros(num)
alpha_relative_error_minpotential = np.zeros(num)
alpha_relative_error_strong_fai = np.zeros(num)
...
for i in range(num):
    ...
    beta1 = beta1_list[i]
    alpha_exact = M/(beta*G*a*b**3)
    alpha_relative_error_strong_dis[i] = np.abs(pred_alpha_array_strong_dis[i][-1]-alpha_exact)/alpha_exact
    alpha_relative_error_minpotential[i] = np.abs(pred_alpha_array_minpotential[i][-1]-alpha_exact)/alpha_exact
    alpha_relative_error_strong_fai[i] = np.abs(pred_alpha_array_strong_fai[i][-1]-alpha_exact)/alpha_exact
    alpha_relative_error_mincomplement[i] = np.abs(pred_alpha_array_mincomplement[i][-1]-alpha_exact)/alpha_exact
    
plt.plot(a_list, alpha_relative_error_strong_dis, marker='o', linestyle='-')
plt.plot(a_list, alpha_relative_error_minpotential, marker='*', linestyle=':')
plt.plot(a_list, alpha_relative_error_strong_fai, marker='v', linestyle='--')
plt.plot(a_list, alpha_relative_error_mincomplement, marker='p', linestyle='-.')
plt.xscale('log')
...
plt.show()
plt.savefig('./PIG/relative_error_all_method.png')
