...
if __name__ == "__main__":
    ...
    x_normalizer = torch.load('./x_normalizer_nu_train%i_test128_data_type3_diff_res' % train_res)
    y_normalizer = torch.load('./y_normalizer_nu_train%i_test128_data_type3_diff_res' % train_res)
    y_normalizer.cuda()
    model = torch.load('./model/TestRes_128_diff_res/TrainRes_%i_diff_res/data_type3/1/FNO_homo_nu' % train_res)
    # 先通过训练数据的来获得一个修正系数，这个类型是150个训练数据
    P = np.zeros([6, 6])
    for i in range(start_train,end_train+1):
        start_time = time.perf_counter()
        mat_filename = r'data_homogenization/matlab_data/FEM results/res128/1801-2400/nu_%i.mat' % i
        ...
        GT_C_path = r'data_homogenization/matlab_data/FEM results/res128/1801-2400/CH_'+str(i)+'.mat'
    ...
    for i in range(start,end+1):
        ...
        print(r'FNO model for prediction of C: error %.4f' % error_C)
    np.save('./outdata/res128_data_type3_diff_res/error_C_data_train%i.npy' % train_res, error_list)
    np.save('./outdata/res128_data_type3_diff_res/error_thermal_data_train%i.npy' % train_res, error_thermal)
    ...
    plt.savefig('./Image/TPMS_forward_res128_type3_diff_res/modulus_homo_res128_data_train%i.png' % train_res, dpi=1000)
    # 显示图形
    plt.show()    
