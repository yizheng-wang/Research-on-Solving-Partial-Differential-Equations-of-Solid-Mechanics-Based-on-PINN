"""
用32,64和128的训练数据，来进行测试集的128的预测，需要有FNO的预测，切片的GT以及reference solution
"""
import os
import numpy as np
import torch
...
def FNO_main(train_data_res, test_data_res, save_index):
    ...
    r_train = (test_data_res-1) // (s_train-1) # r is for interval
    ...
    x_normalizer =  torch.load('x_normalizer_nu_train%i_test%i_data_type3_diff_res' % (train_data_res, test_data_res))
    y_normalizer =  torch.load('y_normalizer_nu_train%i_test%i_data_type3_diff_res' % (train_data_res, test_data_res))
    # y_normalizer.cuda()
    ...
    write_vtk_in('./Image/NTK_res128_diff_res/dis_res%i_data1801-2400_in' % train_data_res, x_space, y_space, z_space, x_test_plot) #input
    
    y_test_p = np.ascontiguousarray(y_test_plot)
    out_p = np.ascontiguousarray(out_plot)
    
    write_vtk_dis('./Image/NTK_res128_diff_res/dis_res%i_data1801-2400_gt1' % train_data_res, x_space, y_space, z_space, x_test_plot, np.ascontiguousarray(y_test_p[..., 0]), np.ascontiguousarray(y_test_p[..., 6]), np.ascontiguousarray(y_test_p[..., 12])) # GT load 1
    write_vtk_dis('./Image/NTK_res128_diff_res/dis_res%i_data1801-2400_gt2' % train_data_res, x_space, y_space, z_space, x_test_plot, np.ascontiguousarray(y_test_p[..., 1]), np.ascontiguousarray(y_test_p[..., 7]), np.ascontiguousarray(y_test_p[..., 13])) # GT load 2
    write_vtk_dis('./Image/NTK_res128_diff_res/dis_res%i_data1801-2400_gt3' % train_data_res, x_space, y_space, z_space, x_test_plot, np.ascontiguousarray(y_test_p[..., 2]), np.ascontiguousarray(y_test_p[..., 8]), np.ascontiguousarray(y_test_p[..., 14])) # GT load 3
    write_vtk_dis('./Image/NTK_res128_diff_res/dis_res%i_data1801-2400_gt4' % train_data_res, x_space, y_space, z_space, x_test_plot, np.ascontiguousarray(y_test_p[..., 3]), np.ascontiguousarray(y_test_p[..., 9]), np.ascontiguousarray(y_test_p[..., 15])) # GT load 4
    write_vtk_dis('./Image/NTK_res128_diff_res/dis_res%i_data1801-2400_gt5' % train_data_res, x_space, y_space, z_space, x_test_plot, np.ascontiguousarray(y_test_p[..., 4]), np.ascontiguousarray(y_test_p[..., 10]), np.ascontiguousarray(y_test_p[..., 16])) # GT load 5
    write_vtk_dis('./Image/NTK_res128_diff_res/dis_res%i_data1801-2400_gt6' % train_data_res, x_space, y_space, z_space, x_test_plot, np.ascontiguousarray(y_test_p[..., 5]), np.ascontiguousarray(y_test_p[..., 11]), np.ascontiguousarray(y_test_p[..., 17])) # GT load 6
    
    write_vtk_dis('./Image/NTK_res128_diff_res/dis_res%i_data1801-2400_FNO1' % train_data_res, x_space, y_space, z_space, x_test_plot, np.ascontiguousarray(out_p[..., 0]), np.ascontiguousarray(out_p[..., 6]), np.ascontiguousarray(out_p[..., 12])) # FNO load 1
    write_vtk_dis('./Image/NTK_res128_diff_res/dis_res%i_data1801-2400_FNO2' % train_data_res, x_space, y_space, z_space, x_test_plot, np.ascontiguousarray(out_p[..., 1]), np.ascontiguousarray(out_p[..., 7]), np.ascontiguousarray(out_p[..., 13])) # FNO load 2
    write_vtk_dis('./Image/NTK_res128_diff_res/dis_res%i_data1801-2400_FNO3' % train_data_res, x_space, y_space, z_space, x_test_plot, np.ascontiguousarray(out_p[..., 2]), np.ascontiguousarray(out_p[..., 8]), np.ascontiguousarray(out_p[..., 14])) # FNO load 3
    write_vtk_dis('./Image/NTK_res128_diff_res/dis_res%i_data1801-2400_FNO4' % train_data_res, x_space, y_space, z_space, x_test_plot, np.ascontiguousarray(out_p[..., 3]), np.ascontiguousarray(out_p[..., 9]), np.ascontiguousarray(out_p[..., 15])) # FNO load 4
    write_vtk_dis('./Image/NTK_res128_diff_res/dis_res%i_data1801-2400_FNO5' % train_data_res, x_space, y_space, z_space, x_test_plot, np.ascontiguousarray(out_p[..., 4]), np.ascontiguousarray(out_p[..., 10]), np.ascontiguousarray(out_p[..., 16])) # FNO load 5
    write_vtk_dis('./Image/NTK_res128_diff_res/dis_res%i_data1801-2400_FNO6' % train_data_res, x_space, y_space, z_space, x_test_plot, np.ascontiguousarray(out_p[..., 5]), np.ascontiguousarray(out_p[..., 11]), np.ascontiguousarray(out_p[..., 17])) # FNO load 6    

    # 下面是训练集的NTK数据
    x_space_train = np.linspace(-0.5, 0.5, train_data_res)
    y_space_train = np.linspace(-0.5, 0.5, train_data_res)
    z_space_train = np.linspace(-0.5, 0.5, train_data_res)
    x_test_plot_train = np.ascontiguousarray(x_test_plot[::r_train,::r_train,::r_train])
    y_test_plot_train = np.ascontiguousarray(y_test_plot[::r_train,::r_train,::r_train])
    write_vtk_in('./Image/NTK_res128_diff_res/train%i_data1801-2400_in' % train_data_res, x_space_train, y_space_train, z_space_train, x_test_plot_train) #input
    
    y_test_train = np.ascontiguousarray(y_test_plot_train)
    
    write_vtk_dis('./Image/NTK_res128_diff_res/train%i_data1801-2400_gt1' % train_data_res, x_space_train, y_space_train, z_space_train, x_test_plot_train, np.ascontiguousarray(y_test_train[..., 0]), np.ascontiguousarray(y_test_train[..., 6]), np.ascontiguousarray(y_test_train[..., 12])) # GT load 1
    write_vtk_dis('./Image/NTK_res128_diff_res/train%i_data1801-2400_gt2' % train_data_res, x_space_train, y_space_train, z_space_train, x_test_plot_train, np.ascontiguousarray(y_test_train[..., 1]), np.ascontiguousarray(y_test_train[..., 7]), np.ascontiguousarray(y_test_train[..., 13])) # GT load 2
    write_vtk_dis('./Image/NTK_res128_diff_res/train%i_data1801-2400_gt3' % train_data_res, x_space_train, y_space_train, z_space_train, x_test_plot_train, np.ascontiguousarray(y_test_train[..., 2]), np.ascontiguousarray(y_test_train[..., 8]), np.ascontiguousarray(y_test_train[..., 14])) # GT load 3
    write_vtk_dis('./Image/NTK_res128_diff_res/train%i_data1801-2400_gt4' % train_data_res, x_space_train, y_space_train, z_space_train, x_test_plot_train, np.ascontiguousarray(y_test_train[..., 3]), np.ascontiguousarray(y_test_train[..., 9]), np.ascontiguousarray(y_test_train[..., 15])) # GT load 4
    write_vtk_dis('./Image/NTK_res128_diff_res/train%i_data1801-2400_gt5' % train_data_res, x_space_train, y_space_train, z_space_train, x_test_plot_train, np.ascontiguousarray(y_test_train[..., 4]), np.ascontiguousarray(y_test_train[..., 10]), np.ascontiguousarray(y_test_train[..., 16])) # GT load 5
    write_vtk_dis('./Image/NTK_res128_diff_res/train%i_data1801-2400_gt6' % train_data_res, x_space_train, y_space_train, z_space_train, x_test_plot_train, np.ascontiguousarray(y_test_train[..., 5]), np.ascontiguousarray(y_test_train[..., 11]), np.ascontiguousarray(y_test_train[..., 17])) # GT load 6
...
if __name__ == "__main__":
    ...
    for i in test_data_resolution:
        input_PATH.append(f'./data_homogenization/matlab_data/FEM results/super/nu_{i}.mat')   # input is 128*128*128 resolution
