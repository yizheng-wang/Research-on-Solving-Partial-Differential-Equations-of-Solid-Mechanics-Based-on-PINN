...
def FNO_main(train_data_res, test_data_res, save_index):
    ...
    output_data = np.swapaxes(np.array(output_data_list).reshape(len(data_number), 128, 128, 128, -1), 2, 3)
    ...
    x_normalizer =  torch.load('x_normalizer_nu_res128_data_type3')
    y_normalizer =  torch.load('y_normalizer_nu_res128_data_type3')
    # y_normalizer.cuda()
    ...
    model_dir = "/model/" + resolution +"/" + 'data_type3/'+ folder_index +"/"
    ...
    write_vtk_in('./Image/NTK_res128/dis_res128_data1801-2400_in', x_space, y_space, z_space, x_test_plot) #input
    
    y_test_p = np.ascontiguousarray(y_test_plot)
    out_p = np.ascontiguousarray(out_plot)
    
    write_vtk_dis('./Image/NTK_res128/dis_res128_data1801-2400_gt1', x_space, y_space, z_space, x_test_plot, np.ascontiguousarray(y_test_p[..., 0]), np.ascontiguousarray(y_test_p[..., 6]), np.ascontiguousarray(y_test_p[..., 12])) # GT load 1
    write_vtk_dis('./Image/NTK_res128/dis_res128_data1801-2400_gt2', x_space, y_space, z_space, x_test_plot, np.ascontiguousarray(y_test_p[..., 1]), np.ascontiguousarray(y_test_p[..., 7]), np.ascontiguousarray(y_test_p[..., 13])) # GT load 2
    write_vtk_dis('./Image/NTK_res128/dis_res128_data1801-2400_gt3', x_space, y_space, z_space, x_test_plot, np.ascontiguousarray(y_test_p[..., 2]), np.ascontiguousarray(y_test_p[..., 8]), np.ascontiguousarray(y_test_p[..., 14])) # GT load 3
    write_vtk_dis('./Image/NTK_res128/dis_res128_data1801-2400_gt4', x_space, y_space, z_space, x_test_plot, np.ascontiguousarray(y_test_p[..., 3]), np.ascontiguousarray(y_test_p[..., 9]), np.ascontiguousarray(y_test_p[..., 15])) # GT load 4
    write_vtk_dis('./Image/NTK_res128/dis_res128_data1801-2400_gt5', x_space, y_space, z_space, x_test_plot, np.ascontiguousarray(y_test_p[..., 4]), np.ascontiguousarray(y_test_p[..., 10]), np.ascontiguousarray(y_test_p[..., 16])) # GT load 5
    write_vtk_dis('./Image/NTK_res128/dis_res128_data1801-2400_gt6', x_space, y_space, z_space, x_test_plot, np.ascontiguousarray(y_test_p[..., 5]), np.ascontiguousarray(y_test_p[..., 11]), np.ascontiguousarray(y_test_p[..., 17])) # GT load 6
    
    write_vtk_dis('./Image/NTK_res128/dis_res128_data1801-2400_FNO1', x_space, y_space, z_space, x_test_plot, np.ascontiguousarray(out_p[..., 0]), np.ascontiguousarray(out_p[..., 6]), np.ascontiguousarray(out_p[..., 12])) # FNO load 1
    write_vtk_dis('./Image/NTK_res128/dis_res128_data1801-2400_FNO2', x_space, y_space, z_space, x_test_plot, np.ascontiguousarray(out_p[..., 1]), np.ascontiguousarray(out_p[..., 7]), np.ascontiguousarray(out_p[..., 13])) # FNO load 2
    write_vtk_dis('./Image/NTK_res128/dis_res128_data1801-2400_FNO3', x_space, y_space, z_space, x_test_plot, np.ascontiguousarray(out_p[..., 2]), np.ascontiguousarray(out_p[..., 8]), np.ascontiguousarray(out_p[..., 14])) # FNO load 3
    write_vtk_dis('./Image/NTK_res128/dis_res128_data1801-2400_FNO4', x_space, y_space, z_space, x_test_plot, np.ascontiguousarray(out_p[..., 3]), np.ascontiguousarray(out_p[..., 9]), np.ascontiguousarray(out_p[..., 15])) # FNO load 4
    write_vtk_dis('./Image/NTK_res128/dis_res128_data1801-2400_FNO5', x_space, y_space, z_space, x_test_plot, np.ascontiguousarray(out_p[..., 4]), np.ascontiguousarray(out_p[..., 10]), np.ascontiguousarray(out_p[..., 16])) # FNO load 5
    write_vtk_dis('./Image/NTK_res128/dis_res128_data1801-2400_FNO6', x_space, y_space, z_space, x_test_plot, np.ascontiguousarray(out_p[..., 5]), np.ascontiguousarray(out_p[..., 11]), np.ascontiguousarray(out_p[..., 17])) # FNO load 6    
...
if __name__ == "__main__":
    ...
    for i in data_number:
        input_PATH.append(f'./data_homogenization/matlab_data/FEM results/res128/1801-2400/nu_{i}.mat')   # input is 128*128*128 resolution
        output_PATH.append(f'./data_homogenization/matlab_data/FEM results/res128/1801-2400/displacement_{i}.mat')   # input is 128*128*128 resolution
    ...
    train_error_dic = np.load('./outdata/res128_data_type3/train_error_dic.npy', allow_pickle=True).item()
    test_error_dic = np.load('./outdata/res128_data_type3/test_error_dic.npy', allow_pickle=True).item()
    time_dic = np.load('./outdata/res128_data_type3/time_dic.npy', allow_pickle=True).item()
    
    train_error_dis_x_1_dic = np.load('./outdata/res128_data_type3/train_error_dis_x_1_dic.npy', allow_pickle=True).item()
    train_error_dis_x_2_dic =np.load('./outdata/res128_data_type3/train_error_dis_x_2_dic.npy', allow_pickle=True).item()
    train_error_dis_x_3_dic =np.load('./outdata/res128_data_type3/train_error_dis_x_3_dic.npy', allow_pickle=True).item()
    train_error_dis_x_4_dic =np.load('./outdata/res128_data_type3/train_error_dis_x_4_dic.npy', allow_pickle=True).item()
    train_error_dis_x_5_dic =np.load('./outdata/res128_data_type3/train_error_dis_x_5_dic.npy', allow_pickle=True).item()
    train_error_dis_x_6_dic =np.load('./outdata/res128_data_type3/train_error_dis_x_6_dic.npy', allow_pickle=True).item()
    
    train_error_dis_y_1_dic = np.load('./outdata/res128_data_type3/train_error_dis_y_1_dic.npy', allow_pickle=True).item()
    train_error_dis_y_2_dic = np.load('./outdata/res128_data_type3/train_error_dis_y_2_dic.npy', allow_pickle=True).item()
    train_error_dis_y_3_dic = np.load('./outdata/res128_data_type3/train_error_dis_y_3_dic.npy', allow_pickle=True).item()
    train_error_dis_y_4_dic = np.load('./outdata/res128_data_type3/train_error_dis_y_4_dic.npy', allow_pickle=True).item()
    train_error_dis_y_5_dic = np.load('./outdata/res128_data_type3/train_error_dis_y_5_dic.npy', allow_pickle=True).item()
    train_error_dis_y_6_dic = np.load('./outdata/res128_data_type3/train_error_dis_y_6_dic.npy', allow_pickle=True).item()
    
    train_error_dis_z_1_dic = np.load('./outdata/res128_data_type3/train_error_dis_z_1_dic.npy', allow_pickle=True).item()
    train_error_dis_z_2_dic = np.load('./outdata/res128_data_type3/train_error_dis_z_2_dic.npy', allow_pickle=True).item()
    train_error_dis_z_3_dic = np.load('./outdata/res128_data_type3/train_error_dis_z_3_dic.npy', allow_pickle=True).item()
    train_error_dis_z_4_dic = np.load('./outdata/res128_data_type3/train_error_dis_z_4_dic.npy', allow_pickle=True).item()
    train_error_dis_z_5_dic = np.load('./outdata/res128_data_type3/train_error_dis_z_5_dic.npy', allow_pickle=True).item()
    train_error_dis_z_6_dic = np.load('./outdata/res128_data_type3/train_error_dis_z_6_dic.npy', allow_pickle=True).item()
    
    test_error_dis_x_1_dic = np.load('./outdata/res128_data_type3/test_error_dis_x_1_dic.npy', allow_pickle=True).item()
    test_error_dis_x_2_dic = np.load('./outdata/res128_data_type3/test_error_dis_x_2_dic.npy', allow_pickle=True).item()
    test_error_dis_x_3_dic = np.load('./outdata/res128_data_type3/test_error_dis_x_3_dic.npy', allow_pickle=True).item()
    test_error_dis_x_4_dic = np.load('./outdata/res128_data_type3/test_error_dis_x_4_dic.npy', allow_pickle=True).item()
    test_error_dis_x_5_dic = np.load('./outdata/res128_data_type3/test_error_dis_x_5_dic.npy', allow_pickle=True).item()
    test_error_dis_x_6_dic = np.load('./outdata/res128_data_type3/test_error_dis_x_6_dic.npy', allow_pickle=True).item()
    
    test_error_dis_y_1_dic = np.load('./outdata/res128_data_type3/test_error_dis_y_1_dic.npy', allow_pickle=True).item()
    test_error_dis_y_2_dic = np.load('./outdata/res128_data_type3/test_error_dis_y_2_dic.npy', allow_pickle=True).item()
    test_error_dis_y_3_dic = np.load('./outdata/res128_data_type3/test_error_dis_y_3_dic.npy', allow_pickle=True).item()
    test_error_dis_y_4_dic = np.load('./outdata/res128_data_type3/test_error_dis_y_4_dic.npy', allow_pickle=True).item()
    test_error_dis_y_5_dic = np.load('./outdata/res128_data_type3/test_error_dis_y_5_dic.npy', allow_pickle=True).item()
    test_error_dis_y_6_dic = np.load('./outdata/res128_data_type3/test_error_dis_y_6_dic.npy', allow_pickle=True).item()
    
    test_error_dis_z_1_dic = np.load('./outdata/res128_data_type3/test_error_dis_z_1_dic.npy', allow_pickle=True).item()
    test_error_dis_z_2_dic = np.load('./outdata/res128_data_type3/test_error_dis_z_2_dic.npy', allow_pickle=True).item()
    test_error_dis_z_3_dic = np.load('./outdata/res128_data_type3/test_error_dis_z_3_dic.npy', allow_pickle=True).item()
    test_error_dis_z_4_dic = np.load('./outdata/res128_data_type3/test_error_dis_z_4_dic.npy', allow_pickle=True).item()
    test_error_dis_z_5_dic = np.load('./outdata/res128_data_type3/test_error_dis_z_5_dic.npy', allow_pickle=True).item()
    test_error_dis_z_6_dic = np.load('./outdata/res128_data_type3/test_error_dis_z_6_dic.npy', allow_pickle=True).item()
    ...
    for training_data_resolution_e in training_data_resolution:
        ...
        plt.yscale("log")
        plt.legend()
        plt.savefig('./Image/TPMS_forward_res128_type3/L2_loss_train_re_dis_1_%i_%i' % (training_data_resolution_e, training_data_resolution_e), dpi=1000)
        plt.show() 
        ...
        plt.yscale("log")
        plt.legend()
        plt.savefig('./Image/TPMS_forward_res128_type3/L2_loss_train_re_dis_2_%i_%i' % (training_data_resolution_e, training_data_resolution_e), dpi=1000)
        plt.show() 
        ...
        plt.yscale("log")
        plt.legend()
        plt.savefig('./Image/TPMS_forward_res128_type3/L2_loss_train_re_dis_3_%i_%i' % (training_data_resolution_e, training_data_resolution_e), dpi=1000)
        plt.show() 
        ...
        plt.yscale("log")
        plt.legend()
        plt.savefig('./Image/TPMS_forward_res128_type3/L2_loss_train_re_dis_4_%i_%i' % (training_data_resolution_e, training_data_resolution_e), dpi=1000)
        plt.show() 
        ...
        plt.yscale("log")
        plt.legend()
        plt.savefig('./Image/TPMS_forward_res128_type3/L2_loss_train_re_dis_5_%i_%i' % (training_data_resolution_e, training_data_resolution_e), dpi=1000)
        plt.show() 
        ...
        plt.yscale("log")
        plt.legend()
        plt.savefig('./Image/TPMS_forward_res128_type3/L2_loss_train_re_dis_6_%i_%i' % (training_data_resolution_e, training_data_resolution_e), dpi=1000)
        plt.show() 
