...
def FNO_main(train_data_res, test_data_res, save_index):
    ...
    # 把所有的40个并行的数据放到一起
    input_data_list = [np.float32(scipy.io.loadmat(input_PATH[i])['data_nu']) for i in range(int(data_number*6))]
    output_data_list = [np.float32(scipy.io.loadmat(output_PATH[i])['real_X']) for i in range(int(data_number*6))]  
    
    input_data = np.array(input_data_list)
    output_data = np.swapaxes(np.array(output_data_list).reshape(len(input_data_list), 64, 64, 64, -1), 2, 3)
    ...
    np.save('./outdata/res64_data_type3_material/shuffled_indices.npy', shuffled_indices)
    # 使用这个索引数组来重新组织原数组
    ...
    torch.save(x_normalizer, 'x_normalizer_nu_res64_data_type3_material')
    torch.save(y_normalizer, 'y_normalizer_nu_res64_data_type3_material')
    ...
    for ep in range(epochs):
        ...
        with torch.no_grad():
            for x, y in test_loader:
                ...
                # every loss for different component
                test_l2_ux_1 += myloss(out[:, :, :, :, 0].view(batch_size,-1), y[:, :, :, :, 0].view(batch_size,-1)).item()
                test_l2_uy_1 += myloss(out[:, :, :, :, 1].view(batch_size,-1), y[:, :, :, :, 1].view(batch_size,-1)).item()
                test_l2_uz_1 += myloss(out[:, :, :, :, 2].view(batch_size,-1), y[:, :, :, :, 2].view(batch_size,-1)).item()
                
                test_l2_ux_2 += myloss(out[:, :, :, :, 3].view(batch_size,-1), y[:, :, :, :, 3].view(batch_size,-1)).item()
                test_l2_uy_2 += myloss(out[:, :, :, :, 4].view(batch_size,-1), y[:, :, :, :, 4].view(batch_size,-1)).item()
                test_l2_uz_2 += myloss(out[:, :, :, :, 5].view(batch_size,-1), y[:, :, :, :, 5].view(batch_size,-1)).item()
            
                test_l2_ux_3 += myloss(out[:, :, :, :, 6].view(batch_size,-1), y[:, :, :, :, 6].view(batch_size,-1)).item()
                test_l2_uy_3 += myloss(out[:, :, :, :, 7].view(batch_size,-1), y[:, :, :, :, 7].view(batch_size,-1)).item()
                test_l2_uz_3 += myloss(out[:, :, :, :, 8].view(batch_size,-1), y[:, :, :, :, 8].view(batch_size,-1)).item()
    
                test_l2_ux_4 += myloss(out[:, :, :, :, 9].view(batch_size,-1), y[:, :, :, :, 9].view(batch_size,-1)).item()
                test_l2_uy_4 += myloss(out[:, :, :, :, 10].view(batch_size,-1), y[:, :, :, :, 10].view(batch_size,-1)).item()
                test_l2_uz_4 += myloss(out[:, :, :, :, 11].view(batch_size,-1), y[:, :, :, :, 11].view(batch_size,-1)).item()
                
                test_l2_ux_5 += myloss(out[:, :, :, :, 12].view(batch_size,-1), y[:, :, :, :, 12].view(batch_size,-1)).item()
                test_l2_uy_5 += myloss(out[:, :, :, :, 13].view(batch_size,-1), y[:, :, :, :, 13].view(batch_size,-1)).item()
                test_l2_uz_5 += myloss(out[:, :, :, :, 14].view(batch_size,-1), y[:, :, :, :, 14].view(batch_size,-1)).item()
                
                test_l2_ux_6 += myloss(out[:, :, :, :, 15].view(batch_size,-1), y[:, :, :, :, 15].view(batch_size,-1)).item()
                test_l2_uy_6 += myloss(out[:, :, :, :, 16].view(batch_size,-1), y[:, :, :, :, 16].view(batch_size,-1)).item()
                test_l2_uz_6 += myloss(out[:, :, :, :, 17].view(batch_size,-1), y[:, :, :, :, 17].view(batch_size,-1)).item()
...
if __name__ == "__main__":
    ...
    for i in range(data_number):
        input_PATH.append(f'./data_homogenization/matlab_data/FEM results/res64/1-600_material/nu_{i+1}.mat')   # input is 128*128*128 resolution
        output_PATH.append(f'./data_homogenization/matlab_data/FEM results/res64/1-600_material/displacement_{i+1}.mat')   # input is 128*128*128 resolution    
    
    for i in range(data_number):
        input_PATH.append(f'./data_homogenization/matlab_data/FEM results/res64/601-1200_material/nu_{i+601}.mat')   # input is 128*128*128 resolution
        output_PATH.append(f'./data_homogenization/matlab_data/FEM results/res64/601-1200_material/displacement_{i+601}.mat')   # input is 128*128*128 resolution
 
    for i in range(data_number):
        input_PATH.append(f'./data_homogenization/matlab_data/FEM results/res64/1201-1800_material/nu_{i+1201}.mat')   # input is 128*128*128 resolution
        output_PATH.append(f'./data_homogenization/matlab_data/FEM results/res64/1201-1800_material/displacement_{i+1201}.mat')   # input is 128*128*128 resolution
 
    for i in range(data_number):
        input_PATH.append(f'./data_homogenization/matlab_data/FEM results/res64/1801-2400_material/nu_{i+1801}.mat')   # input is 128*128*128 resolution
        output_PATH.append(f'./data_homogenization/matlab_data/FEM results/res64/1801-2400_material/displacement_{i+1801}.mat')   # input is 128*128*128 resolution
        
    for i in range(data_number):
        input_PATH.append(f'./data_homogenization/matlab_data/FEM results/res64/2401-3000_material/nu_{i+2401}.mat')   # input is 128*128*128 resolution
        output_PATH.append(f'./data_homogenization/matlab_data/FEM results/res64/2401-3000_material/displacement_{i+2401}.mat')   # input is 128*128*128 resolution 
 
    for i in range(data_number):
        input_PATH.append(f'./data_homogenization/matlab_data/FEM results/res64/3001-3600_material/nu_{i+3001}.mat')   # input is 128*128*128 resolution
        output_PATH.append(f'./data_homogenization/matlab_data/FEM results/res64/3001-3600_material/displacement_{i+3001}.mat')   # input is 128*128*128 resolution
    ...
    ntrain = 3000  # first 1000 of smooth1.mat 
    ntest = 600    # first 100 of smooth1.mat 
    # r = 5
    # h = int(((421 - 1)/r) + 1)
    # s = h
    ...
    np.save('./outdata/res64_data_type3_material/train_error_dic.npy', train_error_dic)
    np.save('./outdata/res64_data_type3_material/test_error_dic.npy', test_error_dic)
    np.save('./outdata/res64_data_type3_material/time_dic.npy', time_dic)
    
    np.save('./outdata/res64_data_type3_material/train_error_dis_x_1_dic.npy', train_error_dis_x_1_dic)
    np.save('./outdata/res64_data_type3_material/train_error_dis_x_2_dic.npy', train_error_dis_x_2_dic)
    np.save('./outdata/res64_data_type3_material/train_error_dis_x_3_dic.npy', train_error_dis_x_3_dic)
    np.save('./outdata/res64_data_type3_material/train_error_dis_x_4_dic.npy', train_error_dis_x_4_dic)
    np.save('./outdata/res64_data_type3_material/train_error_dis_x_5_dic.npy', train_error_dis_x_5_dic)
    np.save('./outdata/res64_data_type3_material/train_error_dis_x_6_dic.npy', train_error_dis_x_6_dic)
    
    np.save('./outdata/res64_data_type3_material/train_error_dis_y_1_dic.npy', train_error_dis_y_1_dic)
    np.save('./outdata/res64_data_type3_material/train_error_dis_y_2_dic.npy', train_error_dis_y_2_dic)
    np.save('./outdata/res64_data_type3_material/train_error_dis_y_3_dic.npy', train_error_dis_y_3_dic)
    np.save('./outdata/res64_data_type3_material/train_error_dis_y_4_dic.npy', train_error_dis_y_4_dic)
    np.save('./outdata/res64_data_type3_material/train_error_dis_y_5_dic.npy', train_error_dis_y_5_dic)
    np.save('./outdata/res64_data_type3_material/train_error_dis_y_6_dic.npy', train_error_dis_y_6_dic)
    
    np.save('./outdata/res64_data_type3_material/train_error_dis_z_1_dic.npy', train_error_dis_z_1_dic)
    np.save('./outdata/res64_data_type3_material/train_error_dis_z_2_dic.npy', train_error_dis_z_2_dic)
    np.save('./outdata/res64_data_type3_material/train_error_dis_z_3_dic.npy', train_error_dis_z_3_dic)
    np.save('./outdata/res64_data_type3_material/train_error_dis_z_4_dic.npy', train_error_dis_z_4_dic)
    np.save('./outdata/res64_data_type3_material/train_error_dis_z_5_dic.npy', train_error_dis_z_5_dic)
    np.save('./outdata/res64_data_type3_material/train_error_dis_z_6_dic.npy', train_error_dis_z_6_dic)
    
    np.save('./outdata/res64_data_type3_material/test_error_dis_x_1_dic.npy', test_error_dis_x_1_dic)
    np.save('./outdata/res64_data_type3_material/test_error_dis_x_2_dic.npy', test_error_dis_x_2_dic)
    np.save('./outdata/res64_data_type3_material/test_error_dis_x_3_dic.npy', test_error_dis_x_3_dic)
    np.save('./outdata/res64_data_type3_material/test_error_dis_x_4_dic.npy', test_error_dis_x_4_dic)
    np.save('./outdata/res64_data_type3_material/test_error_dis_x_5_dic.npy', test_error_dis_x_5_dic)
    np.save('./outdata/res64_data_type3_material/test_error_dis_x_6_dic.npy', test_error_dis_x_6_dic)
    
    np.save('./outdata/res64_data_type3_material/test_error_dis_y_1_dic.npy', test_error_dis_y_1_dic)
    np.save('./outdata/res64_data_type3_material/test_error_dis_y_2_dic.npy', test_error_dis_y_2_dic)
    np.save('./outdata/res64_data_type3_material/test_error_dis_y_3_dic.npy', test_error_dis_y_3_dic)
    np.save('./outdata/res64_data_type3_material/test_error_dis_y_4_dic.npy', test_error_dis_y_4_dic)
    np.save('./outdata/res64_data_type3_material/test_error_dis_y_5_dic.npy', test_error_dis_y_5_dic)
    np.save('./outdata/res64_data_type3_material/test_error_dis_y_6_dic.npy', test_error_dis_y_6_dic)
    
    np.save('./outdata/res64_data_type3_material/test_error_dis_z_1_dic.npy', test_error_dis_z_1_dic)
    np.save('./outdata/res64_data_type3_material/test_error_dis_z_2_dic.npy', test_error_dis_z_2_dic)
    np.save('./outdata/res64_data_type3_material/test_error_dis_z_3_dic.npy', test_error_dis_z_3_dic)
    np.save('./outdata/res64_data_type3_material/test_error_dis_z_4_dic.npy', test_error_dis_z_4_dic)
    np.save('./outdata/res64_data_type3_material/test_error_dis_z_5_dic.npy', test_error_dis_z_5_dic)
    np.save('./outdata/res64_data_type3_material/test_error_dis_z_6_dic.npy', test_error_dis_z_6_dic)