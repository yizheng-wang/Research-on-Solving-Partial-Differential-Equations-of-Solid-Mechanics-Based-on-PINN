# 处理一个127**3的GRF生成的数据，让FNO去处理，首先第一步让vtk变成mat，然后用matlab计算出ground true

# import vtk
# from scipy.io import savemat
# from vtkmodules.util import numpy_support
# # 创建一个VTK读取器来加载你的VTK文件
# reader = vtk.vtkGenericDataObjectReader()
# reader.SetFileName("./data_homogenization/matlab_data/data/GRF/GRF_127.vtk")
# reader.Update()

# # 提取你感兴趣的数据，这里作为示例提取点数据
# # 注意：具体提取哪些数据取决于你的VTK文件内容和需求
# data = reader.GetOutput().GetPointData().GetArray(0)
# data_array = numpy_support.vtk_to_numpy(data)

# # 假设原始数据的维度是127x127x127
# # 重新塑形数组以恢复三维结构
# data_3d = data_array.reshape((127, 127, 127))
# data_3d = data_3d.astype(float)

# # 将2变成0，1变成0.3，这是因为我们几何模型是把有材料的地方设置为nu=0.3，没有材料的地方设置成0
# data_3d[data_3d==2]=0
# # 将数据保存到MAT文件
# savemat("./data_homogenization/matlab_data/data/GRF/GRF_127.mat", {"data": data_3d})
"""
GRF预测
"""
import os
...
def FNO_main(train_data_res, test_data_res, save_index):
    ...
    r_train = (127-1) // (s_train-1) # r is for interval
    
    s_test = test_data_res 
    r_test = (127-1) // (s_test-1) # r is for interval   
    ...
    output_data = np.swapaxes(np.array(output_data_list).reshape(len(data_number), 127, 127, 127, -1), 2, 3)
    ...
    write_vtk_in('./Image/NTK_GRF_res128_type3/GRF_in', x_space, y_space, z_space, x_test_plot) #input
    
    y_test_p = np.ascontiguousarray(y_test_plot)
    out_p = np.ascontiguousarray(out_plot)
    
    write_vtk_dis('./Image/NTK_GRF_res128_type3/GRF_gt1', x_space, y_space, z_space, x_test_plot, np.ascontiguousarray(y_test_p[..., 0]), np.ascontiguousarray(y_test_p[..., 6]), np.ascontiguousarray(y_test_p[..., 12])) # GT load 1
    write_vtk_dis('./Image/NTK_GRF_res128_type3/GRF_gt2', x_space, y_space, z_space, x_test_plot, np.ascontiguousarray(y_test_p[..., 1]), np.ascontiguousarray(y_test_p[..., 7]), np.ascontiguousarray(y_test_p[..., 13])) # GT load 2
    write_vtk_dis('./Image/NTK_GRF_res128_type3/GRF_gt3', x_space, y_space, z_space, x_test_plot, np.ascontiguousarray(y_test_p[..., 2]), np.ascontiguousarray(y_test_p[..., 8]), np.ascontiguousarray(y_test_p[..., 14])) # GT load 3
    write_vtk_dis('./Image/NTK_GRF_res128_type3/GRF_gt4', x_space, y_space, z_space, x_test_plot, np.ascontiguousarray(y_test_p[..., 3]), np.ascontiguousarray(y_test_p[..., 9]), np.ascontiguousarray(y_test_p[..., 15])) # GT load 4
    write_vtk_dis('./Image/NTK_GRF_res128_type3/GRF_gt5', x_space, y_space, z_space, x_test_plot, np.ascontiguousarray(y_test_p[..., 4]), np.ascontiguousarray(y_test_p[..., 10]), np.ascontiguousarray(y_test_p[..., 16])) # GT load 5
    write_vtk_dis('./Image/NTK_GRF_res128_type3/GRF_gt6', x_space, y_space, z_space, x_test_plot, np.ascontiguousarray(y_test_p[..., 5]), np.ascontiguousarray(y_test_p[..., 11]), np.ascontiguousarray(y_test_p[..., 17])) # GT load 6
    
    write_vtk_dis('./Image/NTK_GRF_res128_type3/GRF_FNO1', x_space, y_space, z_space, x_test_plot, np.ascontiguousarray(out_p[..., 0]), np.ascontiguousarray(out_p[..., 6]), np.ascontiguousarray(out_p[..., 12])) # FNO load 1
    write_vtk_dis('./Image/NTK_GRF_res128_type3/GRF_FNO2', x_space, y_space, z_space, x_test_plot, np.ascontiguousarray(out_p[..., 1]), np.ascontiguousarray(out_p[..., 7]), np.ascontiguousarray(out_p[..., 13])) # FNO load 2
    write_vtk_dis('./Image/NTK_GRF_res128_type3/GRF_FNO3', x_space, y_space, z_space, x_test_plot, np.ascontiguousarray(out_p[..., 2]), np.ascontiguousarray(out_p[..., 8]), np.ascontiguousarray(out_p[..., 14])) # FNO load 3
    write_vtk_dis('./Image/NTK_GRF_res128_type3/GRF_FNO4', x_space, y_space, z_space, x_test_plot, np.ascontiguousarray(out_p[..., 3]), np.ascontiguousarray(out_p[..., 9]), np.ascontiguousarray(out_p[..., 15])) # FNO load 4
    write_vtk_dis('./Image/NTK_GRF_res128_type3/GRF_FNO5', x_space, y_space, z_space, x_test_plot, np.ascontiguousarray(out_p[..., 4]), np.ascontiguousarray(out_p[..., 10]), np.ascontiguousarray(out_p[..., 16])) # FNO load 5
    write_vtk_dis('./Image/NTK_GRF_res128_type3/GRF_FNO6', x_space, y_space, z_space, x_test_plot, np.ascontiguousarray(out_p[..., 5]), np.ascontiguousarray(out_p[..., 11]), np.ascontiguousarray(out_p[..., 17])) # FNO load 6    
...
if __name__ == "__main__":
    ...
    training_data_resolution = [127]
    test_data_resolution = [127]
    run_index = 0
    ...
    for i in data_number:
        input_PATH.append(f'./data_homogenization/matlab_data/FEM results/GRF/nu_GRF_{i}.mat')   # input is 128*128*128 resolution
        output_PATH.append(f'./data_homogenization/matlab_data/FEM results/GRF/displacement_GRF_{i}.mat')   # input is 128*128*128 resolution
