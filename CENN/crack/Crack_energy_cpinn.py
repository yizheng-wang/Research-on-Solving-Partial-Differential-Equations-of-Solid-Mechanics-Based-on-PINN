...
def interface1(Ni1): # 内部的边界点,由于交界面附近的梯度很大，所以多配一些点
    ...
    y = np.sin(theta) * rp
    xi = np.stack([x, y], 1)
    xi = torch.tensor(xi, dtype=torch.float32, requires_grad=True, device='cuda')
    return xi
...
def interface(Ni):
    ...
    y = np.sin(theta) * re
    xi = np.stack([x, y], 1)
    xi = torch.tensor(xi, dtype=torch.float32, device='cuda')
    return xi
def train_data(Nb, Nf):
    ...
    Xb = torch.tensor(Xb, dtype=torch.float32, device='cuda') # 转化成tensor
    ...
    Xf1 = torch.tensor(Xf1, dtype=torch.float32, requires_grad=True, device='cuda')
    Xf2 = torch.tensor(Xf2, dtype=torch.float32, requires_grad=True, device='cuda')
...
def pred(xy):
    ...
    Xf2 : tensor
        exterior region.
    '''
    pred = torch.zeros((len(xy), 1), device = 'cuda').double()
    # 上区域的预测，由于要用到不同的特解网络
    pred[(xy[:, 1]>0) ] = model_p1(xy[xy[:, 1]>0]) + \
        RBF(xy[xy[:, 1]>0]) * model_h1(xy[xy[:, 1]>0])
    # 下区域的预测，由于要用到不同的特解网络
    pred[xy[:, 1]<0] = model_p2(xy[xy[:, 1]<0]) + \
        RBF(xy[xy[:, 1]<0]) * model_h2(xy[xy[:, 1]<0])
    ...
    pred[(xy[:, 1]==0) & (xy[:, 0]>0)] = (model_p1(xy[(xy[:, 1]==0) & (xy[:, 0]>0)]) + model_p2(xy[(xy[:, 1]==0) & (xy[:, 0]>0)]))/2  + \
        RBF(xy[(xy[:, 1]==0) & (xy[:, 0]>0)]) * (model_h1(xy[(xy[:, 1]==0) & (xy[:, 0]>0)]) + model_h2(xy[(xy[:, 1]==0) & (xy[:, 0]>0)]))/2
...
def evaluate():
    ...
    xy_test = torch.from_numpy(xy_test).float().cuda() # to the model for the prediction
...
model_h1 = homo(2, 15, 1).cuda()
model_h2 = homo(2, 15, 1).cuda()
criterion = torch.nn.MSELoss()
...
for epoch in range(nepoch_u0):
    ...
    def closure():  
        ...
        u_i1 = model_p1(Xi1)  + RBF(Xi1)*model_h1(Xi1)  # 分别得到两个网络的交界面预测
        u_i2 = model_p2(Xi2)  + RBF(Xi2)*model_h2(Xi2)         
        ...
        if delta != 0: # 这一条必须要加，否则Jie对坐标的导数会出现nan，因为Jie对参数的导数是一个链式导数，其中会涉及到Jie对坐标的导数，而对坐标的导数由于坐标是0，所以会导致分母为0，进而出现nan
            J = J1 + J2 + Jie1 + Jie2
        if delta ==0:
            J = J1 + J2
            
        # 添加交界面的原函数损失
        u_ii1 = model_p1(Xi)  + RBF(Xi)*model_h1(Xi)  # 分别得到两个网络的交界面预测
        u_ii2 = model_p2(Xi)  + RBF(Xi)*model_h2(Xi)  
...
plt.title('cpinn energy prediction') 
plt.colorbar(h2)
plt.subplot(3, 3, 5)
...
plt.suptitle("cpinn energy")
plt.savefig('./picture/cpinn energy.png')
plt.show()
