...
def LP(n, x):
    ...
    ----------
    n : int
        the order of the Legendre polynomials
    x : float, 2d
        the coordinate of input.
        the rows is the number of the data
        the column is the dimension of the data

    Returns
    -------
    TYPE
        the value of the Legendre polynomials in the order of n coresponding to the coordinates.

    '''
    # 写二维data的勒让德多项式
    if n==-1:return torch.zeros(len(x)).cuda()
    if n==0: return torch.ones(len(x)).cuda()
    if n==1: return x
    if n>=2 :
        return (2*n-1)/n*x*LP(n-1, x) - (n-1)/n*LP(n-2, x)
def LPvalue1(npp, xy):
    # 输入二维坐标，输出相应的勒让德正交多项式，注意这里的坐标，是上面的区域，也就是x正常，y是0到1，y需要坐标转化一下
    npp = 5
    out = torch.zeros(len(xy), npp**2).cuda()
    for order1 in range(npp):
        for order2 in range(npp):
            lp1 = LP(order1, xy[:, 0])
            lp2 = LP(order2, xy[:, 1]*2-1)
            lp = lp1*lp2
            out[:, order1*npp + order2] = lp
    return out
...
for epoch in range(nepoch_u0):
    ...
    if epoch%100 == 0:        
        Xb, Xf1, Xf2 = train_data(30, 128)
        Xi1 = interface1(5000)
        Xi2 = interface2(5000)
        Xi = interface(100)
        
        # 计算上述每一个点的权函数的值，这些点包括Xf1, Xf2, 注意不包括Xi1和Xi2
        npp = 5
        Xf1w = LPvalue1(npp, Xf1)
        Xf2w = LPvalue2(npp, Xf2)
    ...
    def closure():  
        ...
        J1 = torch.sum(((du1dxx + du1dyy)*Xf1w)**2)/npp**2 
        J2 = torch.sum(((du2dxx + du2dyy)*Xf2w)**2)/npp**2
        ...
        loss =( b1 * J1 + b2 * J2 + b3 * loss_b1 * len(pred_b1) + b4 * loss_b2 * len(pred_b2) + b5 * loss_bi * len(pred_bi1))/2
        error_t = evaluate()
        optim_h.zero_grad()
