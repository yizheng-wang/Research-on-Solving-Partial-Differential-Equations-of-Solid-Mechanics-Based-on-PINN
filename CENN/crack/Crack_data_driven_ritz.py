...
for epoch in range(nepoch_u0):
    ...
    if epoch%100 == 0:        
        ...
        u_exact1 = torch.sqrt(Xf1[:, 0]**2+Xf1[:, 1]**2)*torch.sqrt((1-Xf1[:, 0]/torch.sqrt(Xf1[:, 0]**2+Xf1[:, 1]**2))/2)
        u_exact1 = u_exact1.unsqueeze(1)
        u_exacti1 = torch.zeros((len(Xi1), 1))
        u_exacti1 = torch.sqrt(Xi1[:, 0]**2+Xi1[:, 1]**2)*torch.sqrt((1-Xi1[:, 0]/torch.sqrt(Xi1[:, 0]**2+Xi1[:, 1]**2))/2)
        u_exacti1 = u_exacti1.unsqueeze(1)
        
        u_exact2 = torch.zeros((len(Xf2), 1))
        u_exact2 = -torch.sqrt(Xf2[:, 0]**2+Xf2[:, 1]**2)*torch.sqrt((1-Xf2[:, 0]/torch.sqrt(Xf2[:, 0]**2+Xf2[:, 1]**2))/2)
        u_exact2 = u_exact2.unsqueeze(1)
        u_exacti2 = torch.zeros((len(Xi2), 1))
        u_exacti2 = -torch.sqrt(Xi2[:, 0]**2+Xi2[:, 1]**2)*torch.sqrt((1-Xi2[:, 0]/torch.sqrt(Xi2[:, 0]**2+Xi2[:, 1]**2))/2)
    ...
    def closure():  
        ...
        u_i1 = model_p1(Xi1)  # 分别得到两个网络的交界面预测
        u_i2 = model_p2(Xi2)         
        ...
        loss = J + loss_b + loss_bi
        
        loss = J.float()  # the functional
        error_t = evaluate()
        optim_h.zero_grad()
        ...
        if epoch%10==0:
            print(' epoch : %i, the loss : %f ,  J1 : %f , J2 : %f , Jie1 : %f, Jie2 : %f, Jb: %f, Ji: %f, J : %f ' % (epoch, loss.data, J1.data, J2.data, Jie1.data, Jie2.data, loss_b.data, loss_bi.data,Je.data))
        return loss
