...
for penalty in penalty_array:
     ...
     def pred(xy): # 从坐标的位置判断用哪个神经网络
         ...
         Xf2 : tensor
             exterior region.
         '''
         pred = torch.zeros((len(xy), 2), device = 'cuda')
         # # 上区域的预测，由于要用到不同的特解网络
         ...
         # 试一试乘以坐标的方式
         # 上区域的预测，由于要用到不同的特解网络
         pred[xy[:, 1]>0.5] = xy[xy[:, 1]>0.5][:,0].unsqueeze(1) * model_h1(xy[xy[:, 1]>0.5])
         # 下区域的预测，由于要用到不同的特解网络
         pred[xy[:, 1]<0.5] = xy[xy[:, 1]<0.5][:,0].unsqueeze(1) * model_h2(xy[xy[:, 1]<0.5])
         # 平局值，即是分片的交界面
         pred[xy[:, 1]==0.5] = xy[xy[:, 1]==0.5][:,0].unsqueeze(1) * (model_h1(xy[xy[:, 1]==0.5]) + model_h2(xy[xy[:, 1]==0.5]))/2
         return pred   
     ...
     model_h1 = homo(2, 15, 2).cuda()
     model_h2 = homo(2, 15, 2).cuda()
     criterion = torch.nn.MSELoss()
     optim_h = torch.optim.Adam(params=chain(model_h1.parameters(), model_h2.parameters()), lr= 0.001)
     ...
     for epoch in range(nepoch_u0):
         ...
         def closure():  
             ...
             # 添加交界面的原函数损失
             u_ii1 = Xi[:,0].unsqueeze(1) * model_h1(Xi)  # 分别得到两个网络的交界面预测
             u_ii2 = Xi[:,0].unsqueeze(1) * model_h2(Xi)  
             Ji = criterion(u_ii1, u_ii2)           
             
             loss = J1 + J2 - external + penalty * Ji # the functional
             optim_h.zero_grad()
             loss.backward()
             ...
             if epoch%10==0:
                 ...
                 print(' epoch : %i, the loss : %f ,  J1 : %f , J2 : %f, Ji : %f, ErrorL2 : %f, Errorvon : %f' % (epoch, loss.data, J1.data, J2.data, Ji.data, errorL2, errorvon))
             return loss
     ...
     filename_out = './output/dem2d/neo+moon_energy_epoch%i_penalty%i_s%i' % (nepoch_u0, penalty, dd) # 输出的VTS文件
     ...
     np.save('./loss_error/hyper_energy_loss_epoch%i_penalty%i_s%i' % (nepoch_u0, penalty, dd), loss_array)
     errorL2_array = np.array(errorL2_array) # make the list to array
     np.save('./loss_error/hyper_energy_errorL2_epoch%i_penalty%i_s%i' % (nepoch_u0, penalty, dd), errorL2_array)
     errorvon_array = np.array(errorvon_array) # make the list to array
     np.save('./loss_error/hyper_energy_errorvon_epoch%i_penalty%i_s%i' % (nepoch_u0, penalty, dd), errorvon_array)
     ...
     femdismag = femdismag[(femcoor[:, 0]==2)]
     femvonx2 = femvon[(femcoor[:, 0]==2)]
     # 接下来我们要中性面上的位移
     femcoormid = femcoor[(femcoor[:, 1]==0.5)] # 只要用到x的坐标就行了
     femdisyy = femdisy[(femcoor[:, 1]==0.5)]
     femdisyyx2 = femdisy[(femcoor[:, 0]==2)]
         
     energyvtu = meshio.read('./output/dem2d/neo+moon_energy_epoch%i_penalty%i_s%i.vtu' % (nepoch_u0, penalty, dd)) 
     ######################################################################################################
     ...
     energydismag = energydismag[(energycoor[:, 0]==2)] 
     energyvon =  energyvon[(energycoor[:, 0]==2)]
     energydisyy = energydisy[(energycoor[:, 1]==0.5)]
     ...
     plt.scatter(femcoorx2[:, 1], femdismag, ls = '--', label = 'fem mag') # 先画参照解
     plt.plot(energycoorx2[:, 1], energydismag, ls = ':', label = 'energy mag') # 先画参照解
     ...
     plt.xlabel('y')
     plt.ylabel('dis')
     plt.title('dismag', fontsize = 20) 
     
     plt.subplot(2, 3, 5)  # 画中线的von大小
     plt.scatter(femcoorx2[:, 1], femvonx2, ls = '--', label = 'fem') # 先画参照解
     plt.plot(energycoorx2[:, 1], energyvon, ls = ':', label = 'energy') # 先画参照解
     plt.legend( loc = 'upper right')
     plt.xlabel('y')
     plt.ylabel('von')
     plt.title('von', fontsize = 20) 
     ...
     plt.scatter(femcoormid[:, 0], femdisyy, ls = '--', label = 'fem') # 先画参照解
     plt.plot(energycoormid[:, 0], energydisyy, ls = ':', label = 'energy') # 先画参照解
     plt.legend( loc = 'upper right')
     plt.xlabel('x')
     plt.ylabel('dis')
     plt.title('disy', fontsize = 20) 
     
     plt.suptitle('CENN hyper%i_penalty%i' % (nepoch_u0, penalty),   size = 25)    
 
     
     plt.savefig('./loss_error/hyper_energy_errorvon_epoch%i_penalty%i_s%i.pdf' % (nepoch_u0, penalty, dd))
    # plt.show()