...
optim_h2 = torch.optim.Adam(params=model_h2.parameters(), lr= 0.001) # 0.001比较好，神经网络2
...
for epoch_inter in range(nepoch_inter): # 两个神经网络交错优化
    ...
    for epoch in range(nepoch_u2): # 两个一起优化
        if epoch ==1000:
            end = time.time()
    ...
    for epoch in range(nepoch_u1): #固定网络2，优化网络2
        ...
        def closure():  
            ...
            loss_array.append((loss+loss2_array[-1]).data.cpu())
            error_array.append(error_t.data.cpu())
...
cost = end - start
print('the consuming time is %f' % cost)
n_test = 100
...
write_arr2DVTK('./output_ntk/pred_energy_cenn', dom_koch_n, u_pred, 'pred_energy_cenn')

write_arr2DVTK('./output_ntk/error_energy_cenn', dom_koch_n, error, 'error_energy_cenn')

plt.subplot(3, 3, 2)
plt.yscale('log')
plt.grid(axis='y')
plt.axhline(y=1.10, color='r', ls = ':') # 画出精确解
plt.plot(loss1_array, ls = '--')
plt.legend(['exact', 'cenn'], loc = 'upper right')
plt.xlabel('the iteration')
plt.ylabel('loss')
plt.title('cenn internal', fontsize = 20) 

plt.subplot(3, 3, 3)
plt.yscale('log')
plt.grid(axis='y')
plt.axhline(y=436.45, color='r', ls = ':') # 画出精确解
plt.plot(loss2_array, ls = '--')
plt.legend(['exact', 'cenn'], loc = 'upper right')
plt.xlabel('the iteration')
plt.ylabel('loss')
plt.title('cenn external', fontsize = 20) 
...
plt.subplot(3, 3, 7) # 做x=0的位移导数预测 dudy
n_test = 100
x0 = np.zeros((n_test, 2))
x0[:, 1] = np.linspace(-np.sqrt(3), np.sqrt(3), 100)
...
plt.subplot(3, 3, 8) # 做y=0的位移导数预测 dudy
n_test = 100
y0 = np.zeros((n_test, 2))
y0[:, 0] = np.linspace(-1, 1, 100)
...
plt.suptitle("cenn_energy")
plt.savefig('./picture/energy_cenn_plane.png')
plt.show()
