...
def setup_domain(): # 四分之一2D方板圆孔
    ...
    X, cells = dmsh.generate(geo, lambda pts: np.abs(c.dist(pts))/15+0.20, tol=1.0e-15) # 划分693个点
    ...
    bcl_u_pts_left = np.zeros((len(lin_y), 2))
    bcl_u_pts_left[:, 1] = lin_y # 做边界的左边点
    ...
    bcl_u_pts_down = np.zeros((len(lin_x), 2))
    bcl_u_pts_down[:, 0] = lin_x # 做边界的下边点
    ...
    lin_y_r = np.linspace(y_min, x_dom[1], x_dom[2])
    bcr_t_pts = np.zeros((len(lin_y_r), 2))
    bcr_t_pts[:, 0] = 20.0
    bcr_t_pts[:, 1] = lin_y_r # 做边界的左边点
    ...
    return dom, boundary_neumann, boundary_dirichlet # 多输出一个本质边界条件位置的ndarray：bound_id
