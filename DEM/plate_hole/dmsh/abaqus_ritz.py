...
from dem_hyperelasticity.plate_hole.dmsh import define_structure as des
...
from dem_hyperelasticity.plate_hole.dmsh import config as cf
    ...
    def train_model(self, shape, dxdydz, data, neumannBC, dirichletBC, iteration, learning_rate, penalty_boundary_loss=5000):
        ...
        for t in range(iteration):
            ...
            def closure():
                ...
                ust_pred.double()
                storedEnergy = self.energy.getStoredEnergy(ust_pred, x) # 获得能量密度
...
if __name__ == '__main__':
    ...
    tablex0_l = datax0.sheet_by_index(1) # 创造线性单元的表
    x0 = tablex0.col_values(0)
    x0 = np.array(x0)
    x0_l = tablex0_l.col_values(0) # 线性单元的x=0坐标
    x0_l = np.array(x0_l)
    x0_2d = np.zeros((len(x0), 2))
    x0_2d[:, 1] = x0 
    x0_2d_l = np.zeros((len(x0_l), 2))
    x0_2d_l[:, 1] = x0_l 
    disy_abqusx0 = tablex0.col_values(1)
    mise_abaqusx0 = tablex0.col_values(2)    
    disy_abqusx0_l = tablex0_l.col_values(1)
    mise_abaqusx0_l = tablex0_l.col_values(2)  

    datay0 = xlrd.open_workbook("hole_abaqus_y=0.xlsx") # 获得x=0的数据，需要提取y方向位移以及mise应力
    tabley0 = datay0.sheet_by_index(0)
    tabley0_l = datay0.sheet_by_index(1) # 创造线性单元的表
    y0 = tabley0.col_values(0)
    y0 = np.array(y0)
    y0_l = tabley0_l.col_values(0) # 线性单元的x=0坐标
    y0_l = np.array(y0_l)
    y0_2d = np.zeros((len(y0), 2))
    y0_2d[:, 0] = y0
    y0_2d_l = np.zeros((len(y0_l), 2))
    y0_2d_l[:, 1] = y0_l 
    disx_abqusy0 = tabley0.col_values(1)  
    disx_abqusy0_l = tabley0_l.col_values(1)
...
#        util.write_vtk_v2p(cf.filename_out, datatest, U, S11, S12, S13, S22, S23, S33, E11, E12, E13, E22, E23, E33, SVonMises)
    ...
    plt.show()
    plt.plot(x0, disy_abqusx0, color = 'red', label = 'reference solution', lw = 2)
    plt.plot(x0, pred_disy_r, color = 'blue', label = 'deep Nitsche ritz solution', lw = 2)
    plt.plot(x0_l, disy_abqusx0_l, color = 'orange', label = 'abaqus_linear_points:2396', lw = 2)
    #for idx, bp in enumerate(penalty_boundary_loss_array):
    #    plt.plot(x0, pred_disy_array[idx], label = 'p%i solution' % bp )
    plt.xlabel('y coordinate') 
    plt.ylabel('dis y')
    plt.title('dis y : x=0, number points : %i' % len(dom))
    plt.legend()
    plt.show()
    # 第二幅是x0的mise应力
    plt.plot(x0, mise_abaqusx0, color = 'red', label = 'reference solution', lw = 2)
    plt.plot(x0, pred_mise_r, color = 'blue', label = 'deep Nitsche ritz solution', lw = 2)
    plt.plot(x0_l, mise_abaqusx0_l, color = 'orange', label = 'abaqus_linear_points:2396', lw = 2)
    #for idx, bp in enumerate(penalty_boundary_loss_array):
    #    plt.plot(x0, pred_mise_array[idx], label = 'p%i solution' % bp )
    plt.xlabel('y coordinate') 
    plt.ylabel('mise')
    plt.title('mise : x=0, number points : %i' % len(dom))
    plt.legend()
    plt.show()
    # 第三幅是y0的x方向位移
    plt.plot(y0, disx_abqusy0, color = 'red', label = 'reference solution', lw = 2)
    plt.plot(y0, pred_disx_r, color = 'blue', label = 'deep Nitsche ritz solution', lw = 2)
    plt.plot(y0_l, disx_abqusy0_l, color = 'orange', label = 'abaqus_linear_points:2396', lw = 2)
    #for idx, bp in enumerate(penalty_boundary_loss_array):
    #    plt.plot(y0, pred_disx_array[idx], label = 'p%i solution' % bp )
    plt.xlabel('x coordinate') 
    plt.ylabel('dis x')
    plt.title('dis x : y=0, number points : %i' % len(dom))
    plt.legend()
    plt.show()
