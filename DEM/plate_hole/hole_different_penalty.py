...
from dem_hyperelasticity.plate_hole import Plate_hole_define_structure as des
...
#        util.write_vtk_v2p(cf.filename_out, datatest, U, S11, S12, S13, S22, S23, S33, E11, E12, E13, E22, E23, E33, SVonMises)
    ...
    plt.show()
    plt.plot(x0, disy_abqusx0, color = 'red', label = 'Abaqus参照解', lw = 1)
    ...
    plt.show()
    # 第二幅是x0的mise应力
    plt.plot(x0, mise_abaqusx0, color = 'red', label = 'abaqus solution', lw = 1)
    for idx, bp in enumerate(penalty_boundary_loss_array):
    ...
    plt.show()
    # 第三幅是y0的x方向位移
    plt.plot(y0, disx_abqusy0, color = 'red', label = 'abaqus solution', lw = 1)
    for idx, bp in enumerate(penalty_boundary_loss_array):
