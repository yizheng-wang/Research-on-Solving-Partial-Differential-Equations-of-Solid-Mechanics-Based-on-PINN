...
class DeepEnergyMethod:
    ...
    def __init__(self, model, numIntType, energy, dim, defect=False, r=0):
        ...
        self.lossArray = []
        self.r = r
        self.particular = MultiLayerNet_p(model[0], model[1], 1).to(dev)  # 定义一个特解的网络结构
        self.distance1 = MultiLayerNet_d(model[0], model[1], 1).to(dev)  # 定义一个距离的网络结构
        self.distance2 = MultiLayerNet_d(model[0], model[1], model[2]).to(dev)  # 定义一个距离的网络结构
    ...
    def train_distance(self, dirichletBC, dom, proportion=0.05, tol=10):
        ...
        criterion = torch.nn.L1Loss(reduction = 'sum')
        optimizer1 = torch.optim.Adam(self.distance1.parameters(), lr = 1e-4) # 一个输出，一个左边界的距离
        optimizer2 = torch.optim.Adam(self.distance2.parameters(), lr = 1e-4) # 另一个输出，一个下边界的距离
        ...
        y_train1= torch.from_numpy(d1).float().to(dev) # 训练两个距离网络
        y_train2= torch.from_numpy(d2).float().to(dev)
        ...
        while True: # 将最近的两个损失进行对比，如果差不多就可以停止迭代了
            ...
            it_time = time.time()
            t += 1
            y_pred1 = self.distance1(D_train)
            loss1 = criterion(y_pred1, y_train1)
            optimizer1.zero_grad()
            loss1.backward()
            optimizer1.step()
            
            y_pred2 = self.distance2(D_train)
    ...
    def getUST(self, x):
        ...
        '''
        ust = self.particular(x) + torch.self.distance1(x) * self.model(x) # 位移是0的时候位移解就是零，所以这里不需要特解，令其为0
...
if __name__ == '__main__':
    ...
    dem.train_distance(boundary_dirichlet, dom, proportion=0.05, tol=10.0)    
    shape = [cf.Nx, cf.Ny]
    dxdy = [cf.hx, cf.hy]
    cf.iteration = 10000
