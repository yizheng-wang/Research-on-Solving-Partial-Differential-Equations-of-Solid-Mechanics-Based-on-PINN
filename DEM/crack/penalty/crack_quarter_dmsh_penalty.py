...
import define_structure_quarter as des
...
class DeepEnergyMethod:
    ...
    def train_model(self, data, neumannBC, dirichletBC, boundary_id, LHD, iteration, learning_rate, penalty_boundary_loss):
        ...
        Jacob = torch.from_numpy(data[:, 2, np.newaxis]).float().to(dev).requires_grad_(True)
        ...
        for t in range(iteration):
            ...
            def closure():
                ...
                internal2 = self.intLoss.ele2d(strainEnergy, Jacob)# 获得了数值积分值
    ...
    def getUST(self, x):
        ...
        '''
        ust = self.model(x)
        Ux = ust[:, 0] # 如果x坐标是0的话，x方向位移也是0,这里由于加了罚函数，所以不用乘以坐标了
...
if __name__ == '__main__':
    ...
    for i in range(1):
        ...
        penalty_boundary_loss = 50000+10000 * (i)
        penalty_array.append(penalty_boundary_loss)
        ...
        dxdy = [cf.hx, cf.hy]
        cf.iteration = 8000
        cf.filename_out = "./output/dem/P%iCrack_Elasticity_minpotential_2Layer_mesh200x200_iter10000_simpsonlr0.0001ADAM" % penalty_boundary_loss
        ...
        pred_mise_r[i] = SVonMisesy0_r
        
    for i in range(1):
        plt.plot(y0, pred_mise_r[i], label = r'DEM罚因子%i' % penalty_array[i])
