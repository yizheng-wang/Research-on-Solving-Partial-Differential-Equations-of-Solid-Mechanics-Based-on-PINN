...
#from DEM.crack.penalty import define_structure as des # uniform
import define_structure as des # dmsh
from DEM.MultiLayerNet import * 
from DEM import EnergyModel as md
from DEM import Utility as util
from DEM.crack.penalty import config as cf
...
if __name__ == '__main__':
    ...
    for i in range(k):
        ...
        penalty_boundary_loss = 10000+10000 * (i)
    ...
    for i in range(k):
        plt.plot(y0, pred_mise_r[i], label = r'DEM罚因子%i' % penalty_array[i], marker='*', ls = ':', ms=5,)
    plt.plot(y0, mise_abaqusy0, label = r'Abaqus(参照解)', color = 'red', marker='o', ms=4,)
