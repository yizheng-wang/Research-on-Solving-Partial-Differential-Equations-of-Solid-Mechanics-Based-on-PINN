...
from DEM.crack.config_quarter import *
import dmsh
import numpy as np

def setup_domain(): # 四分之一2D方板裂纹
    geo = dmsh.Rectangle(0.0, 20.0, 0.0, 20.0)
    p1 = dmsh.Circle([10.0, 0.0], 0.0)
    #X, cells = dmsh.generate(geo, 0.2 , tol=1.0e-10)
    X, cells = dmsh.generate(geo, lambda pts: 0.13 * p1.dist(pts)+ 0.2 , tol=1.0)
    ...
    bound_id = np.ones((Nx * Ny, 2)) # 生成一个储存本质边界条件的ndarray
    c = 0
    for x in np.nditer(lin_x):
        ...
        dom[tb:te, 0] = x
        dom[tb:te, 1] = lin_y
    #crack_pts_idx = np.where((dom[:, 1] == y_min)&(dom[:, 0]<10))
    #dom[crack_pts_idx, 1] = Height/Nx*0.8

    print(dom.shape) # dom已经生成并且赋值了
    ...
    bcl_u_pts_idx_down = np.where((dom[:, 1] == y_min)&(dom[:, 0]>=10))
    bcl_u_pts_down = dom[bcl_u_pts_idx_down, :][0] # 找到下边界不是裂纹的点的坐标
    ...
    bcr_t_pts_idx = np.where(dom[:, 1] == Height) # 返回一个元组
    bcr_t_pts = dom[bcr_t_pts_idx, :][0] # 由于返回一个元组，所以生成的是一个3维的ndarray，第一个维度是由于bcr_t_pts_idx是元组才形成的，因此后面要加[0]
    bcr_t = np.ones(np.shape(bcr_t_pts)) * [known_up_tx, known_up_ty]

    ax.scatter(dom_dmsh[:, 0], dom_dmsh[:, 1], s=0.05, facecolor='blue')
    ...
    return dom_dmsh, dom_dis, boundary_neumann, boundary_dirichlet, bound_id # 多输出一个本质边界条件位置的ndarray：bound_id


# -----------------------------------------------------------------------------------------------------
# prepare inputs for testing the model
# -----------------------------------------------------------------------------------------------------
def get_datatest(Nx=num_test_x, Ny=num_test_y): # 这里比方板圆孔更加简单，不用删除二范数小于半径的点
    '''
...
if __name__ == '__main__':
    dom_dmsh, dom_dis, boundary_neumann, boundary_dirichlet, bound_id=setup_domain()
