...
class DeepEnergyMethod:
    ...
    def train_distance(self, dirichletBC, dom, proportion=0.05, tol=10):
        ...
        d21 =    np.zeros((len(B_train1), 1))  # 左边界点距离左边界点的距离，自然是0
        d22, _ = kdt2.query(B_train1, k=1, return_distance = True) # 左边界点距离下边界的距离
        d31, _ = kdt1.query(B_train2, k=1, return_distance = True) # 下边界点距离左边界的距离
        d32 =    np.zeros((len(B_train2), 1)) # 下边界距离下边界的距离
        # 将上面的距离组合起来
        d1 = np.concatenate((d11, d21, d31))
        d2 = np.concatenate((d12, d22, d32))
        d =np.concatenate((d1, d2), 1)/10 # 归一化
    ...
    #     '''
    #     x = x 
    #     dis = torch.zeros((len(x), 2)) # dis是二维，为了distance乘以广义网络做准备
    #     segt1 = torch.tensor([[0., 0., 0., 20.]]).float().to(dev)
    #     segt2 = torch.tensor([[10., 0., 20., 0.]]).float().to(dev)
    #     dis[:, 0] = self.phi(x, segt1)[:,0] # 获得每一个x点距离本质边界条件的距离,这里返回是一个二维张量，为了赋值[:, 0]
    #     dis[:, 1] = self.phi(x, segt2)[:,0] # 获得每一个x点距离本质边界条件的距离
    #     return dis
    ...
    def train_model(self, data, neumannBC, dirichletBC, boundary_id, LHD, iteration, learning_rate, penalty_boundary_loss):
        ...
        for t in range(iteration):
            ...
            def closure():
                ...
                internal2 = self.intLoss.montecarlo2D(strainEnergy, LHD[0], LHD[1], r=self.r)# 获得了数值积分值
    ...
    def getUST(self, x):
        ...
        '''
        ust = self.particular(x) + self.distance(x) * self.model(x) # 位移是0的时候位移解就是零，所以这里不需要特解，令其为0
    ...
    def evaluate_model_c(self, datatest):
        ...
        if dim == 2:
            ...
            xy_tensor = xy_tensor.to(dev)
            xy_tensor.requires_grad_(True)
            ust_pred_torch = self.getUST(xy_tensor) # 裂纹处是y=0的地方，用第二个神经网络预测就行了
            duxdxy = grad(ust_pred_torch[:, 0].unsqueeze(1), xy_tensor, torch.ones(xy_tensor.size()[0], 1, device=dev),
                           create_graph=True, retain_graph=True)[0]
            duydxy = grad(ust_pred_torch[:, 1].unsqueeze(1), xy_tensor, torch.ones(xy_tensor.size()[0], 1, device=dev),
                           create_graph=True, retain_graph=True)[0]
    ...
    def plot_distance_contourf(self, dom): # plot distance contourf
        dom_tensor = torch.from_numpy(dom).float().to(dev)
        dis = self.distance(dom_tensor)
        disx = dis[:,0].data.cpu().numpy().reshape(int(len(dom)**0.5), int(len(dom)**0.5))
        disy = dis[:,1].data.cpu().numpy().reshape(int(len(dom)**0.5), int(len(dom)**0.5))
        domx = dom[:,0].reshape(int(len(dom)**0.5), int(len(dom)**0.5))
        domy = dom[:,1].reshape(int(len(dom)**0.5), int(len(dom)**0.5))
        plt.contourf(domx, domy, disx)
        plt.colorbar()
        plt.show()
        
        plt.contourf(domx, domy, disy)
        plt.colorbar()
        plt.show()
        
    def plot_distance(self, dom): # plot distance prediton with the comparision of the exact distance on the essential boundary.
        bl_idx = np.where(dom[:,0]==0)
        bd_idx = np.where((dom[:,0]>=10) & (dom[:,1]==0))
        bl = dom[bl_idx]
        bd = dom[bd_idx]
        bl_tensor = torch.from_numpy(bl).float().to(dev)
        bd_tensor = torch.from_numpy(bd).float().to(dev)
        dis_bl = self.distance(bl_tensor)[:, 0].data.cpu().numpy()
        dis_bd = self.distance(bd_tensor)[:, 1].data.cpu().numpy()

        plt.plot(bl[:, 1], dis_bl)
        plt.show()
        
        plt.plot(bd[:, 0], dis_bd)
        plt.show()     
    ...
    def linseg(self, x,y,x1,y1,x2,y2):
        ...
        return phi
    def phi(self, x, segments):
        m = 1.
        R = 0. 
        for i in range(len(segments[:,0])): 
            phi = self.linseg(x[:,0].unsqueeze(1), x[:,1].unsqueeze(1),segments[i,0],segments[i,1],segments[i,2],segments[i,3]) 
            R = R + 1./phi**m
        R = 1/R**(1/m) 
        return R
...
if __name__ == '__main__':
    ...
    datatest = des.get_datatest() # dataset没有原点，因为原点的应变是奇异的
    # ----------------------------------------------------------------------
    #                   STEP 2: SETUP MODEL
    # ----------------------------------------------------------------------
    mat = md.EnergyModel('elasticityMP', 2, cf.E, cf.nu)
    dem = DeepEnergyMethod([cf.D_in, cf.H, cf.D_out], 'simpson', mat, 2) # 2是二维的意思
    # ----------------------------------------------------------------------
    #                   STEP 3: TRAINING MODEL
    # ----------------------------------------------------------------------
    start_time = time.time()
    dem.train_particular_soluton(boundary_dirichlet)
    # the points of distance in dom are meshed uniformly
    dom_dis_x = np.linspace(0, 20, 50)
    dom_dis_y = np.linspace(0, 20, 50)
    dom_dis_x, dom_dis_y = np.meshgrid(dom_dis_x, dom_dis_y)
    dom_dis = np.stack([dom_dis_x.flatten(), dom_dis_y.flatten()], 1)
    dem.train_distance(boundary_dirichlet, dom_dis, proportion=1, tol=30.0)   
    dem.plot_distance_contourf(dom)
    dem.plot_distance(dom)
    # plot distance contourf
    shape = [cf.Nx, cf.Ny]
    dxdy = [cf.hx, cf.hy]
    cf.iteration = 100000
    ...
    datatest = np.concatenate((datatest, z), 1) # parafiew需要3维数据才可以，所以增添了一维
    ...
    plt.plot(y0, pred_mise_r, label = 'DEM')
    plt.plot(y0, mise_abaqusy0, label = r'Abaqus(参照解)')
    plt.title('mise: y=0 x>10, number points: %i' % len(dom))
