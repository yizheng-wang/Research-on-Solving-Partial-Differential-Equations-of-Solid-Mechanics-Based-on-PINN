...
def setup_domain(): # 2D悬臂梁撒点
    ...
    bcr_u_pts_idx_right = np.where(dom[:, 0] == Length)
    bcr_u_pts_right = dom[bcr_u_pts_idx_right, :][0] 
    bcr_u_right = np.ones(np.shape(bcr_u_pts_right)) * [known_right_tx, known_right_ty]
    ...
    ax.scatter(bcr_u_pts_right[:, 0], bcr_u_pts_right[:, 1], s=0.5, facecolor='red')
    ...
    boundary_dirichlet = {
        ...
        "neumann_right": {
            "coord": bcr_u_pts_right,
            "known_value": bcr_u_right,
