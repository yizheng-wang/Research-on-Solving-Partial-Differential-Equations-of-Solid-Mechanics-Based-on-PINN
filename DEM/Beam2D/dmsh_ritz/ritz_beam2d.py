...
from dem_hyperelasticity.Beam2D.dmsh_ritz import define_structure as des
...
from dem_hyperelasticity.Beam2D.dmsh_ritz import config as cf
...
class DeepEnergyMethod:
    ...
    def train_model(self, shape, dxdydz, data, neumannBC, dirichletBC, iteration, learning_rate, penalty_boundary_loss=10):
        x = torch.from_numpy(data[:, 0:2]).float()
        x = x.to(dev)
        x.requires_grad_(True)
        Jacob = torch.from_numpy(data[:, 2, np.newaxis]).float()
        Jacob = x.to(dev)
        Jacob.requires_grad_(True)
        #Jacob.requires_grad_(True)
        ...
        for t in range(iteration):
            ...
            def closure():
                ...
                internal2 = self.intLoss.ele2d(storedEnergy, Jacob)
...
if __name__ == '__main__':
    ...
    for d_e in d_list:
        ...
        # ----------------------------------------------------------------------
        dom, boundary_neumann, boundary_dirichlet = des.setup_domain(s=d_e) # 返回的boundary value是一个字典，里面不仅仅有坐标还有力大小，力大小没有标准化
        # ----------------------------------------------------------------------
        #                   STEP 2: SETUP MODEL
        # ----------------------------------------------------------------------
...
#        dem.train_distance(boundary_dirichlet, dom) # 由于悬臂梁的距离网络很容易，所以自己给定了
    ...
    abaqus_y = np.ones(len(points_num))*(-1.33528)
    plt.plot(points_num, abaqus_y, color = 'red', label = 'abaqus solution')
    plt.plot(points_num, pred_num, label = 'deep Nitsche ritz', )
    plt.xlabel('the number of the points')
    plt.ylabel('y displacement')
    plt.title('the max y displacement')
    plt.legend()
    plt.show()
