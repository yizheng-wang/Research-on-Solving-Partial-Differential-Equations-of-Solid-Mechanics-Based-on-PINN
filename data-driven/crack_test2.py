...
for i in range(len(k_list)):
    ...
    def evaluate_u(N_test): # evaluate the L2 error of the displacement  x and y 
        ...
        dudr = grad(u_pred, rt_test, torch.ones(rt_test.size()[0], 1).cuda(), retain_graph=True, create_graph=True)[0]
        u_pred = dudr.flatten().data.cpu().numpy()
    ...
    def evaluate_singular_u(N_test=5):# calculate the prediction of the strain
        ...
        dudr = grad(u, rt_test, torch.ones(rt_test.size()[0], 1).cuda(), retain_graph=True, create_graph=True)[0]
      
        u_pred = dudr.data.cpu().numpy()
        rt_test = rt_test.data.cpu().numpy()
        r = rt_test[:, 0]
