...
for i in range(len(k_list)):
    ...
    def dom_data(Nf):
        ...
        theta = np.linspace(0, np.pi, Nf)
        
        r = np.linspace(delta, a, Nf) 
        r = r**k/(a**k)*a
        r_mesh, theta_mesh = np.meshgrid(r, theta)
        rt_dom = np.stack([r_mesh.flatten(), theta_mesh.flatten()], 1)
    ...
    def bound_data(Nf): # 用来评估误差，所以不用太接近0
        ...
        rt_bound = np.stack([r.flatten(), theta.flatten()], 1)
    ...
    def evaluate_e(N_test):# evaluate the L2 error of strain xx, yy, and xy
        rt_test = bound_data(N_test)
        u = pred(rt_test)
    
        dudrt = grad(u, rt_test, torch.ones(rt_test.size()[0], 1).cuda(), retain_graph=True, create_graph=True)[0]
        dudt_pred = dudrt[:, 1]
      
        e_yy_pred = dudt_pred.data.cpu().numpy()/(rt_test[:, 0].data.cpu().numpy())
    
        
        rt_test = rt_test.data.cpu().numpy()
        r = rt_test[:, 0]
    
        e_yy_exact = 0.5/r**0.5
    ...
    def evaluate_singular_strain(N_test=5):# calculate the prediction of the strain
    ...
    for epoch in range(nepoch):
        ...
        def closure():  
            ...
            error_eyy_array[i].append(erroreyy)
            if epoch%10==0:
            ...
            return loss
        optim.step(closure)
    r_mesh,   e_yy_pred[i]= evaluate_strain(N_test)
    r, e_yy_singular_pred[i]   = evaluate_singular_strain(4)
...
# strain plotting
# =============================================================================
e_yy_exact = 0.5/r_mesh**0.5
    
plt.plot(r_mesh, e_yy_exact,  label = r'$\varepsilon_{yy}$解析解')
# for i in range(len(k_list)):
#     plt.plot(r_mesh, e_yy_pred[i], linestyle='--', label = r'$\varepsilon_{yy}$预测解 k=%i' % k_list[i])
for i in range(len(dom_num_list)):
    plt.plot(r_mesh, e_yy_pred[i], linestyle='--', label = r'$\varepsilon_{yy}$预测解 dom=%i' % dom_num_list[i])
plt.xlabel('x')
...
plt.yscale('log')
for i in range(len(k_list)):
    plt.plot(error_eyy_array[i], linestyle=':', label = 'dom=%i' % dom_num_list[i])
plt.xlabel('迭代数')
plt.ylabel('误差')
plt.legend()
settick()
...
for i in range(len(k_list)):
    y_pred = np.log10(e_yy_singular_pred[i])
