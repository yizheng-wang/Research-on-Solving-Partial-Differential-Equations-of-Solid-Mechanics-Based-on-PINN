...
def train_data(Nb, Nf):
    ...
    plt.xlim(-square_size, square_size)
    plt.ylim(-square_size, square_size)
    plt.title("Triangular Mesh for Square Domain [-1, 1]")
    plt.xlabel("x")
    plt.ylabel("y")
    plt.grid(True)
    plt.show()
    ...
    Xf1 = torch.tensor(Xf1, requires_grad=True, device='cuda').float()
    Xf2 = torch.tensor(Xf2, requires_grad=True, device='cuda').float()
...
for epoch in range(nepoch_u0):
    ...
    scheduler.step()
torch.save(model_h, './model/DEM_MLP_rbf/DEM_MLP_rbf_tri')
    
np.save( './results/DEM_MLP_rbf/error_tri.npy', np.array(error_array))    
...
np.save( './results/DEM_MLP_rbf/interface_tri.npy', inter_strain) # 将DEM rbf的奇异应变储存起来
...
plt.suptitle("DEM_MLP_rbf")
plt.savefig('./results/DEM_MLP_rbf/DEM_MLP_rbf_tri.pdf', dpi = 300)
plt.show()
...
plt.xlabel('x')
plt.ylabel('y')
plt.savefig('./results/DEM_MLP_rbf/abs_error_DEM_MLP_rbf_tri.pdf', dpi = 300)
plt.show()  
