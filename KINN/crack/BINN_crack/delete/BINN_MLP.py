...
def draw_colormap(x, y, fvalue,title='',mode=0):
    ...
    plt.ylabel('y')
    if mode == 1: # save fig
        plt.savefig('../results/BINN_MLP/abs_error_BINN_MLP.pdf', dpi = 300)
    plt.show()  
...
def plot_inner(Grids0,Net,N,area=np.array([0,0,2,2]),device='cpu'):
    ...
    U_pred=Grids0.inner(Net,xy_test,device).detach().cpu().numpy().reshape(N, N)
...
if __name__=='__main__': 
    ...
    if trainmode==True:
        ...
        loss_array, error_array = Bie_trainer_rizzo_again.train_partitial_rizzo_again(Grids0,Bienet,device,workspace,2500,0.001) 
        torch.save(Bienet, '../model/BINN_MLP/BINN_MLP')
        np.save( '../results/BINN_MLP/error.npy', error_array)
...
#%%
    # N_test = 100
    # interx = np.linspace(0, 1, N_test)[1:-1] # 获得interface的测试点
    # intery = np.zeros(N_test)[1:-1]
    # inter = np.vstack([interx, intery]).T
    # inter_t = torch.tensor(inter.astype(np.float32)).requires_grad_(True).cuda()
    # pred_inter = Grids0.inner(Bienet,inter,device).requires_grad_(True)
    # dudxyi = grad(pred_inter.unsqueeze(1), inter_t, torch.ones(len(inter), 1).cuda())[0]
    ...
    # plt.title('BINN about e32 on interface')
    # plt.legend()
    # np.save( '../results/BINN_MLP/inter_strain.npy', np.array(dudyi.detach().cpu()))   