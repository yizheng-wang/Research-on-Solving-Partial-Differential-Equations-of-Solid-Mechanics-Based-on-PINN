...
def train_partitial_rizzo_again(Grids0,Net1,device,workspace,Nepoch,lr0):
    ...
    trainerr=np.empty([0],dtype=float)
    testerr=np.empty([0],dtype=float)
    ...
    for epoch in range(Nepoch):  
        ...
        loss_array=np.append(loss_array,loss.cpu().detach().numpy())
        error = plot_inner(Grids0,Net1,100,area = np.array([0,0.5,2,1]),device = device)
        if epoch%10==0:
            print(' epoch : %i, the loss : %f ,  error: %f' % (epoch, loss.data, error))
        if epoch%50==49:
            Grids0.update_func(Net1)
            # plt.figure()
...
def plot_inner(Grids0,Net,N,area=np.array([-1,0,1,1]),device = 'cpu'):
    ...
    U=Grids0.inner(Net,x,device = device).cpu().numpy()
