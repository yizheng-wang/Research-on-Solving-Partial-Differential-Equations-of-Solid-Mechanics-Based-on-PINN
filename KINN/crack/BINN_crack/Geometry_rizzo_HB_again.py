...
Ngauss= 10
class grids_HB():
    def __init__(self,p1,p2,func,type0,device='cpu'):
        '''
        Parameters
        ----------
        p1 : TYPE
        ...
        self.normT_gp = torch.tensor(self.norm.repeat(Ngauss,axis=0)).float()
        self.normT_loggp = torch.tensor(self.norm.repeat(2*Ngauss_log,axis=0)).float()
    ...
    def update_func(self,Net):
        ...
        self.df=torch.sum(torch.autograd.grad(self.f,self.GPT,grad_outputs=gradient,
                               ...
                               create_graph=True,
                               only_inputs=True,
                               allow_unused=True)[0]*self.normT_gp,axis=1).reshape([-1,1])
        ...
        self.df_log=torch.sum(torch.autograd.grad(GP_f2,self.GPTlog,grad_outputs=gradient2,
                               ...
                               create_graph=True,
                               only_inputs=True,
                               allow_unused=True)[0]*self.normT_loggp.float(),axis=1).reshape([-1,1])
...
def Generatepoints(x,y,b,h,func,NE = 100,device='cpu'):
    ...
    Grids = grids_HB(p0, p1, func,BCtype,device)
