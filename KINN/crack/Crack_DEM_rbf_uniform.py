...
for epoch in range(nepoch_u0):
    ...
    scheduler.step()
torch.save(model_h, './model/DEM_MLP_rbf/DEM_MLP_rbf')
    
np.save( './results/DEM_MLP_rbf/error.npy', np.array(error_array))    
...
np.save( './results/DEM_MLP_rbf/interface.npy', inter_strain) # 将DEM rbf的奇异应变储存起来
...
plt.savefig('./results/DEM_MLP_rbf/DEM_MLP_rbf.pdf', dpi = 300)
plt.show()
...
plt.xlabel('x')
plt.ylabel('y')
plt.savefig('./results/DEM_MLP_rbf/abs_error_DEM_MLP_rbf.pdf', dpi = 300)
plt.show()  
