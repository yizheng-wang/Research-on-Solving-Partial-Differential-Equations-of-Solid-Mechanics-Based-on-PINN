...
model = KAN([2, 5,5, 1], base_activation=torch.nn.SiLU, grid_size=3, grid_range=[-1.0, 1.0], spline_order=2).cuda()
criterion = torch.nn.MSELoss()
...
for epoch in range(nepoch):
    ...
    if epoch == 0:
        dom_koch_n = koch_points.get_koch_points(10000).astype(np.float32) # 获得n_test个koch的随机分布点
        #dom_area_koch_n = koch_points.generate_triangle_grid(150).astype(np.float32) # 三角形网格
        # dom_koch_n =dom_area_koch_n[:,:-1]
        # area = dom_area_koch_n[:,2, np.newaxis].astype(np.float32)
        dom_koch_t= torch.tensor(dom_koch_n,  requires_grad=True, device = 'cuda') # 将numpy数据转化为tensor
        # area_koch_t = torch.tensor(area).to('cuda')
    ...
    def closure():  
        ...
        # J = torch.sum(((0.5 * a * (dudx**2 + dudy**2)) - f*u_pred)*area_koch_t)
        J = torch.sum(0.5 * a * (dudx**2 + dudy**2))*10*np.sqrt(3)/3/len(dom_koch_t) 
...
write_arr2DVTK('./output_ntk/pred_DEM_KAN', dom_koch_n, u_pred, 'pred_energy')

write_arr2DVTK('./output_ntk/error_DEM_KAN', dom_koch_n, error, 'error_energy')
...
np.save('./results/koch_DEM_KAN_error.npy', error_array)
plt.yscale('log')
...
plt.subplot(3, 3, 7) # 做y=0的位移导数预测 dudx
n_test = 100
y0 = np.zeros((n_test, 2)).astype(np.float32)
y0[:, 0] = np.linspace(-1, 1, 100).astype(np.float32)
x = y0[:, 0][:, np.newaxis]
...
plt.suptitle("DEM")
plt.savefig('./pic/DEM_kan_plane.png')
plt.show()
