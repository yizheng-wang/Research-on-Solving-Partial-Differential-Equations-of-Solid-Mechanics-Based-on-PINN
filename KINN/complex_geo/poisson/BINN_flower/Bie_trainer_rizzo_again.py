...
def train_partitial_rizzo_again(Grids0,Net1,device,workspace,Nepoch,lr0):
    optimizer = torch.optim.Adam(Net1.parameters(), lr=lr0, betas=(0.9, 0.999), eps=1e-08, weight_decay=0, amsgrad=False)
    scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[3000, 5000, 7000, 10000, 13000, 15000, 18000], gamma = 0.50)
    #先计算基本解的值，这部分在训练过程中一直保持不变
...
def train_partitial_rizzo_again_efficient_mode(Grids0,Net1,device,workspace,Nepoch,lr0,Netinvname):
    optimizer = torch.optim.Adam(Net1.parameters(), lr=lr0, betas=(0.9, 0.999), eps=1e-08, weight_decay=0, amsgrad=False)
    # scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=(Nepoch), gamma=0.3)
    scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[3000, 5000, 7000], gamma = 0.1)
    #先计算基本解的值，这部分在训练过程中一直保持不变
...
def get_inner_error(Grids0,Net,N,area = np.array([-1,0,1,1])):
    ...
    return error_t
def plot_inner(Grids0,Net,N,area=np.array([-1,0,1,1])):
