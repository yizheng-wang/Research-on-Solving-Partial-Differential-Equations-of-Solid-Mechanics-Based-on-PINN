...
def generate_triangle_grid(number):
    points = get_koch_points_lin(number)
    ...
    # 删除不再koch内部的点
    label = whether_koch(dom_point)
    filtered_points = dom[label]


    plt.scatter(filtered_points[:, 0], filtered_points[:, 1], c='red')
    plt.show()
    return filtered_points


# # 生成并绘制三角形网格
# dom = generate_triangle_grid(100)