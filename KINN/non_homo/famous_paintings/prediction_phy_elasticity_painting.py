...
for name_e in name_list:
    ...
    T_data = np.cos(15*np.pi*X*Y)
    
    
    image_path = './painting/' + name_painting + '_bw.jpg'
    K_data = load_image(image_path) # 这就是最后要得到的热传导系数
    F_data = np.cos(15*np.pi*X*Y)*((15*np.pi*X)**2 + (15*np.pi*Y)**2)*K_data
    # =============================================================================
    # # bulid coordinate to the K
    # =============================================================================
    model = KAN([2, 15, 15, 1], base_activation=torch.nn.SiLU, grid_size=100, grid_range=[0, 1.0], spline_order=3).cuda()
    ...
    def evaluate_model(model, epoch):
        ...
        # reference solution
        h1 = plt.contourf(X, Y, K_data, levels=100 ,cmap = 'gray')
        ax = plt.gca()
        ax.set_aspect(1)
        plt.colorbar(h1).ax.set_title('Reference')
        plt.xlabel('x')
        plt.ylabel('y')
        plt.savefig(f'./PINNs_results/Ref_{name_painting}_epoch{epoch}.pdf', dpi=300)
        plt.show()      
        
    
        # kan
        h1 = plt.contourf(X, Y, K_pred, levels=100 ,cmap = 'gray')
        ax = plt.gca()
        ax.set_aspect(1)
        plt.colorbar(h1).ax.set_title('KAN_pred')
        plt.xlabel('x')
        plt.ylabel('y')
        plt.savefig(f'./PINNs_results/KAN_pred_{name_painting}_epoch{epoch}.pdf', dpi=300)
        plt.show() 
    
        h1 = plt.contourf(X, Y, error, levels=100 ,cmap = 'gray')
        ax = plt.gca()
        ax.set_aspect(1)
        plt.colorbar(h1).ax.set_title('KAN_error')
        plt.xlabel('x')
        plt.ylabel('y')
        plt.savefig(f'./PINNs_results/KAN_error_{name_painting}_epoch{epoch}.pdf', dpi=300)
        plt.show() 
    ...
    for epoch in range(num_epochs):
        ...
        K_pred_tensor = model(XY_tensor).reshape(N_u, N_u) # torch.tensor(K_data, dtype=torch.float32).cuda() # 
        # 自动满足边界条件
        ...
        phy_loss = (K_pred_tensor * Txxyy + F_tensor)**2
    ...
    torch.save(model.state_dict(), './models/model_PINNs_kan.pth')
    
    
    np.save('PINNs_results/' + name_painting + '_kan_loss.npy', loss_list)
    np.save('PINNs_results/' + name_painting + '_kan_l2.npy', error_list)
    
    # Plot training error
    plt.figure(figsize=(10, 6))
    plt.plot(loss_list, 'b-', label='KAN Error')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.yscale('log')
    plt.legend()
    plt.title('Error evolution')
    plt.savefig('./PINNs_results/loss_' + name_painting +  '.pdf', dpi=300)
    plt.show()
