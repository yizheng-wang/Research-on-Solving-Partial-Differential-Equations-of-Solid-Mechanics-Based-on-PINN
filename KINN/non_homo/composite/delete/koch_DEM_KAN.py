...
model = KAN([2, 5, 1], base_activation=torch.nn.SiLU, grid_size=2, grid_range=[-1.0, 1.0], spline_order=3).cuda()
...
for epoch in range(nepoch): # cpinn先优化1
    ...
    def closure():  
        ...
        J1 =  ((0.5 * a1 * torch.sum(du1dx**2 + du1dy**2)) - torch.sum(f1*u_pred1)) * (r0**2*np.pi)/len(du1dx)
        
        J2 =  ((0.5 * a2 * torch.sum(du2dx**2 + du2dy**2)) - torch.sum(f2*u_pred2)) * (2*2-(r0**2*np.pi))/len(du2dx)
...
write_arr2DVTK('./output_ntk/pred_dem_kan', dom_koch_n, u_pred, 'pred_pinn')

write_arr2DVTK('./output_ntk/error_dem_kan', dom_koch_n, error, 'error_pinn')
