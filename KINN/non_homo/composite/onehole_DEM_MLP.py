...
# model = KAN([2, 5,5,5, 1], base_activation=torch.nn.SiLU, grid_size=15, grid_range=[-1.0, 1.0], spline_order=3).cuda()
criterion = torch.nn.MSELoss()
...
write_arr2DVTK('./output_ntk/DEM_MLP/pred_dem_mlp', dom_koch_n, u_pred, 'pred_DEM')

write_arr2DVTK('./output_ntk/DEM_MLP/error_dem_mlp', dom_koch_n, error, 'error_DEM')
...
np.save('./results/DEM_MLP/error_L2.npy', error_array)
...
np.save('./results/DEM_MLP/x0_u_pred.npy', x0_pred_save)
x0_exact_save = np.vstack([x0[:, 1], exactx0.flatten()]).T
np.save('./results/DEM_MLP/x0_u_exact.npy', x0_exact_save)
...
np.save('./results/DEM_MLP/x0_dudy_pred.npy', x0dudy_pred_save)
x0dudy_exact_save = np.vstack([x0[:, 1], exactdx0.flatten()]).T
np.save('./results/DEM_MLP/x0_dudy_exact.npy', x0dudy_exact_save)
...
plt.suptitle("pinn")
plt.savefig('./pic/dem_mlp_plane.png')
plt.show()
...
np.save('./results/DEM_MLP/x0_u_z_pred.npy', u_x0_z_pred_save)
x0_exact_save = np.vstack([Y_u_05_z, exact_u_x0_z.flatten()]).T
np.save('./results/DEM_MLP/x0_u_z_exact.npy', x0_exact_save)
