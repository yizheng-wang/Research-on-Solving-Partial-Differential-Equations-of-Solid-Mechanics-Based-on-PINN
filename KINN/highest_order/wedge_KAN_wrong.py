...
#     KAN strong form
# =============================================================================
model = KAN([1,5, 1], base_activation=torch.nn.SiLU, grid_size=5, grid_range=[0, 1.0], spline_order=3).cuda()
...
for epoch in range(nepoch):
    ...
    def closure():  
        ...
        dfdtheta5 = grad(dfdtheta4, theta, torch.ones(theta.size()[0], 1).cuda(), retain_graph=True, create_graph=True)[0]
        dfdtheta6 = grad(dfdtheta5, theta, torch.ones(theta.size()[0], 1).cuda(), retain_graph=True, create_graph=True)[0]
        dfdtheta7 = grad(dfdtheta6, theta, torch.ones(theta.size()[0], 1).cuda(), retain_graph=True, create_graph=True)[0]
        dfdtheta8 = grad(dfdtheta7, theta, torch.ones(theta.size()[0], 1).cuda(), retain_graph=True, create_graph=True)[0]
        dfdtheta9 = grad(dfdtheta8, theta, torch.ones(theta.size()[0], 1).cuda(), retain_graph=True, create_graph=True)[0]
        dfdtheta10 = grad(dfdtheta9, theta, torch.ones(theta.size()[0], 1).cuda(), retain_graph=True, create_graph=True)[0]
        dfdtheta11 = grad(dfdtheta10, theta, torch.ones(theta.size()[0], 1).cuda(), retain_graph=True, create_graph=True)[0]
        dfdtheta12 = grad(dfdtheta11, theta, torch.ones(theta.size()[0], 1).cuda(), retain_graph=True, create_graph=True)[0]
