...
def plot_inner(Grids0,Net,N,mode=0):
    ...
    draw_colormap(x, u, filename = '../results/BINN_MLP/abs_error_dis_BINN_MLP.pdf', title='abs error',  mode=1)
    draw_colormap(x, SVonMises, filename = '../results/BINN_MLP/abs_error_mise_BINN_MLP.pdf',  title='abs error', mode=1)
    write_vtk_v2p('../results/BINN_MLP/Plate_hole_BINNs', x, u)  
...
if __name__=='__main__': 
    ...
    # #                     [0,0]])
    # BCtype = np.array([[0,0]])
    # Grids0 = Geometry_elastic_more_geometry.circle(x,y,r,func,BCtype,20,E=1,v=0.3)a
    #------------------------------------------------
    #多几何测试 孔板算例
    # func = elastic_linear
    # x=0
...
#------------------------------------------------------------ 
    ...
    line1 = {'mode':'line','NE':30,'x0':[5,0],'x1':[20,0],'type':[1,0],'func':elastic_01}
    line2 = {'mode':'line','NE':40,'x0':[20,0],'x1':[20,20],'type':[1,1],'func':elastic_01}
    line3 = {'mode':'line','NE':40,'x0':[20,20],'x1':[0,20],'type':[1,1],'func':elastic_01}
    line4 = {'mode':'line','NE':30,'x0':[0,20],'x1':[0,5],'type':[0,1],'func':elastic_01}
    arc1 = {'mode':'arc','NE':20,'c':[0,0],'theta1':np.pi/2,'theta2':0,'type':[1,1],'r':5,'func':elastic_01}
    geoset = [line1,line2,line3,line4,arc1]
    Grids0 = Geometry_elastic_more_geometry.Any_HB(geoset,elastic_01,E=1000,v=0.3,device = device)
 ...
 #------------------------------------------------------------     
    ...
    if trainmode==True:
        ...
        filename_out =  '../results/BINN_MLP'
        
        np.save(filename_out+'/exact_x0disy.npy', x0disy_fem)
        np.save(filename_out+'/BINN_x0disy.npy', x0disy_pred)
        
        np.save(filename_out+'/exact_x0mise.npy', x0mise_fem)
        np.save(filename_out+'/BINN_x0mise.npy', x0mise_pred)
        
        np.save(filename_out+'/exact_y0disx.npy', y0disx_fem)
        np.save(filename_out+'/BINN_y0disx.npy', y0disx_pred)
        ...
        # U,Uac=plot_inner(Grids0,Bienet,300,mode=1)