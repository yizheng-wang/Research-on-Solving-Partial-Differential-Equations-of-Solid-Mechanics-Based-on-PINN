...
if __name__ == '__main__':
    ...
    dem = DeepEnergyMethod([2,5,5,5, 2], 'montecarlo', mat, 2, defect=True, r = cf.r)
    # ----------------------------------------------------------------------
    ...
    dxdy = [cf.hx, cf.hy]
    cf.iteration = 2500
    cf.filename_out = "./output/KINN_DEM"
    ...
    #*****************************************************************************************************************************************
    util.write_vtk_v2p(cf.filename_out+'/Plate_hole_KINN_DEM', datatest, U, S11, S12, S13, S22, S23, S33, E11, E12, E13, E22, E23, E33, SVonMises)
    surUx, surUy, surUz = U
...
#  compare the disy and mises(x=0), and disx (y=0)
    ...
    # 分析x=0这条线
    U_2dx0_r, S11, S12, S13, S22, S23, S33, E11, E12, E13, E22, E23, E33, SVonMisesx0_r, F11, F12, F21, F22 = dem.evaluate_model(x0_2d)
    pred_disy_r = U_2dx0_r[1]
    pred_mise_r = SVonMisesx0_r
    # 分析y=0这条线
    U_2dy0_r, S11, S12, S13, S22, S23, S33, E11, E12, E13, E22, E23, E33, SVonMises, F11, F12, F21, F22 = dem.evaluate_model(y0_2d)
    ...
    np.save(cf.filename_out+'/KINN_DEM_x0disy.npy', x0disy_pred)
    
    np.save(cf.filename_out+'/exact_x0mise.npy', x0mise_fem)
    np.save(cf.filename_out+'/KINN_DEM_x0mise.npy', x0mise_pred)
    
    np.save(cf.filename_out+'/exact_y0disx.npy', y0disx_fem)
    np.save(cf.filename_out+'/KINN_DEM_y0disx.npy', y0disx_pred)