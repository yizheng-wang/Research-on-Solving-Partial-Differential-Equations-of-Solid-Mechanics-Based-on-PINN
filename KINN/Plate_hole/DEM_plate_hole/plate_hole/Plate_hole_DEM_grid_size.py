...
class DeepEnergyMethod:
    ...
    def __init__(self, model, numIntType, energy, dim, defect=False, r=0):
        # self.data = data
        self.model = MultiLayerNet_P(model)
...
if __name__ == '__main__':
    ...
    error_L2_list = []
    error_H1_list = []
    layers_list = [[2,5,2],[2,15,2],[2,30,2],[2,5,5,2],[2,15,15,2],[2,30,30,2],[2,5,5,5,2],[2,15,15,15,2],[2,30,30,30,2]]
    dict_scale_law = {}
    for layer in layers_list:
    #dem = DeepEnergyMethod([cf.D_in, cf.H, cf.D_out], 'montecarlo', mat, 2, defect=True, r = cf.r)
        dem = DeepEnergyMethod(layer, 'montecarlo', mat, 2, defect=True, r = cf.r)
        # ----------------------------------------------------------------------
        ...
        dxdy = [cf.hx, cf.hy]
        cf.iteration = 50000
        cf.filename_out = '../../results/DEM_rbf'
        # 将坐标点先存起来
        ...
        num_para = sum(p.numel() for p in dem.model.parameters())
    ...
    np.save( '../../results/plate_hole_DEM_grid_size.npy', dict_scale_law)
    loaded_data = np.load('../../results/plate_hole_DEM_grid_size.npy', allow_pickle=True).item()
    print(loaded_data)    
