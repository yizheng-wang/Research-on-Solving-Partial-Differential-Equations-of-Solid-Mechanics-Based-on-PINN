...
class DeepEnergyMethod:
    # Instance attributes
    def __init__(self, model, numIntType, energy, dim, modeltype='mlp'):
        # self.data = data
        if modeltype == 'mlp':
            self.model = MultiLayerNet(model[0], model[1], model[2])
            self.model = self.model.to(dev)
        if modeltype == 'kan':
            self.model = KAN(width=[model[0], model[1], model[1], model[2]], grid=10, k=3, grid_eps=1.0, noise_scale_base=0.25, grid_range=[0, 4])
            self.model = self.model.to(dev)            
