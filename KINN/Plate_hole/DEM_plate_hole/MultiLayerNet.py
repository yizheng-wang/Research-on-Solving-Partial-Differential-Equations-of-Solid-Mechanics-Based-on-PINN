...
class MultiLayerNet_P(torch.nn.Module):
    def __init__(self, layers):
        ...
        # Create the linear layers based on the input list
        for i in range(len(layers) - 1):
            self.layers.append(nn.Linear(layers[i], layers[i + 1]))
            torch.nn.init.normal_(self.layers[-1].bias, mean=0, std=1)
            torch.nn.init.normal_(self.layers[-1].weight, mean=0, std=np.sqrt(2 / (layers[i] + layers[i + 1])))
