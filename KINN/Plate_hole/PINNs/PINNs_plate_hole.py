...
def train(model, optimizer, epochs, dom_p, boundary_p):
    U_mag_loss_array = []
    Mise_loss_array = []
    U_mag_exact = np.ascontiguousarray(np.load('../abaqus_reference/onehole/U_mag.npy')[:,1])
    MISES_exact = np.ascontiguousarray(np.load('../abaqus_reference/onehole/MISES.npy')[:,1])
