...
def getUST(x):
    ...
    Ux =  model1(x_scale) # 如果x坐标是0的话，x方向位移也是0
    Uy =  model2(x_scale) # 如果y坐标是0的话，y方向位移也是0
...
# Training
def train_data(model1, model2, optimizer1,  optimizer2, epochs, dom_p):
    ...
    x_scale = xy_tensor/Length
    
    start_time = time.time()
    # train dis_x first
    for epoch in range(epochs):
        optimizer1.zero_grad()
        ust_x_pred_torch = model1(x_scale)
        if epoch % 1000 == 0:
            end_time = time.time()
            print('Time is ' + str(end_time-start_time))
        loss1 = criterion(ust_x_pred_torch.flatten(), U_x_exact_tensor)
        loss1.backward()
        optimizer1.step()
    ...
    for epoch in range(epochs):
        ...
        ust_y_pred_torch = model2(x_scale)
        if epoch % 1000 == 0:
            end_time = time.time()
            print('Time is ' + str(end_time-start_time))
        loss2 = criterion(ust_y_pred_torch.flatten(), U_y_exact_tensor)
        loss2.backward()
        optimizer2.step()

        if epoch % 100 == 0:
            #print(f'Epoch {epoch}, L_balance: {loss_balance.item()}, Ll_dis: {loss_l_x.item()}, Ll_f: {loss_l_y.item()}, Ld_dis: {loss_d_y.item()}, Ld_f: {loss_d_x.item()}, Lr: {loss_r.item()}, Lu: {loss_u.item()}, Lc: {loss_c.item()}')
            print(f'Epoch {epoch}, loss2: {loss2.item()}')
...
if __name__ == '__main__':
    ...
    # model1 = KAN([2, 5,5, 1], base_activation=torch.nn.SiLU, grid_size=10, grid_range=[0, 1.0], spline_order=3).cuda()
    # model2 = KAN([2, 5,5, 1], base_activation=torch.nn.SiLU, grid_size=10, grid_range=[0, 1.0], spline_order=3).cuda()
    
    model1 = MultiLayerNet(2,30,1).cuda()
    model2 = MultiLayerNet(2,30,1).cuda()
    ...
    #train_data(model1, model2, optimizer1,  optimizer2, epochs_data, dom_p)

   # 保存模型
    #torch.save(model1.state_dict(), './model/model1_mlp.pth')
    #torch.save(model2.state_dict(), './model/model2_mlp.pth')
    
    # 加载模型
    model1.load_state_dict(torch.load('./model/model1_mlp.pth'))
    model2.load_state_dict(torch.load('./model/model2_mlp.pth'))

    E = torch.tensor(1100.,  requires_grad=True, device='cuda') # true 1000
    nu = 0.3
    G = E/2/(1+nu)
    D11_mat = E/(1-nu**2)
    D22_mat = E/(1-nu**2)
    ...
    filename_out =  '../results/PINNs_MLP_inverse/'
