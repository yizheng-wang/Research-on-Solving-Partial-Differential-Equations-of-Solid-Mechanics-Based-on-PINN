...
def fine_hyper(losses, model):

    # Clear previous gradients
    model.zero_grad()
    avg_grad_all = []
    # Accumulate gradients for each loss component
    for loss in losses:
        ...
        avg_grad_all.append(avg_grad)
    # 接下来算最大的梯度
    max_grad = 0.0
    losses[0].backward(retain_graph=True) # 第一个损失函数是PDEs
    for param in model.parameters():
            if param.grad is not None:
                max_grad = max(max_grad, param.grad.abs().max().item())
                
    hyper = torch.tensor(avg_grad_all[0])/torch.tensor(avg_grad_all)
    hyper[0] = 1
...
def train(model, optimizer, epochs, dom_p, boundary_p):
    hyper_d = 1
    hyper_l_x = 1 # x方向的左侧位移，这个容易很低，所以放大一些
    hyper_l_y = 1
    hyper_d_x = 1
    hyper_d_y = 1 # y方向的位移，这个容易很低，所以放大一些
    hyper_r = 1
    hyper_u = 1
    hyper_c = 1
    ...
    for epoch in range(epochs):
        ...
        loss_hyper = [loss_balance, loss_l_x, loss_l_y, loss_d_x, loss_d_y, loss_r, loss_u, loss_c]
        # 调整超参数
        if epoch % 500==0:
            hyper = fine_hyper(loss_hyper, model)
            hyper_d = hyper[0]
            hyper_l_x = hyper[1]
            hyper_l_y = hyper[2]
            hyper_d_x = hyper[3]
            ...
            hyper_u = hyper[6]
            hyper_c = hyper[7]
            print(f'Hyper: {hyper_d},{hyper_l_x},{hyper_l_y},{hyper_d_x},{hyper_d_y},{hyper_r},{hyper_u},{hyper_c}')
        loss.backward()
...
if __name__ == '__main__':
    ...
    model = MultiLayerNet(2,30,2).cuda()
    Radius = 5.
    Length = 20.
    ...
    torch.save(model, '../model/PINNs_MLP/PINNs_MLP.pth')
...
#  compare the disy and mises(x=0), and disx (y=0)
    ...
    np.save(filename_out+'/PINN_x0disy.npy', x0disy_pred)
    
    np.save(filename_out+'/exact_x0mise.npy', x0mise_fem)
    np.save(filename_out+'/PINN_x0mise.npy', x0mise_pred)
    
    np.save(filename_out+'/exact_y0disx.npy', y0disx_fem)
    np.save(filename_out+'/PINN_y0disx.npy', y0disx_pred)
